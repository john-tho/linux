...
#define PLL_REG(x)	((unsigned long) pll_base + (x))
...
static void __iomem *pll_base = NULL;
...
static void flush_hornet_uart_fifo(void) {
	unsigned uart_cs = (unsigned) ioremap(0x18020004, PAGE_SIZE);
...
static int __init rb400_beep_init(void) {
	...
	beep_phys = virt_to_phys(beep_buf);
...
static void beep_rb400(unsigned freq) {
	...
	if (freq == 0) {
	    ...
	    arch_sync_dma_for_device(beep_phys, beep_buf_size, DMA_TO_DEVICE);
	}
	else {
	    fill_buffer(beep_buf, beep_buf_size, freq, sample_freq);
	    arch_sync_dma_for_device(beep_phys, beep_buf_size, DMA_TO_DEVICE);
	    rb400_writel((unsigned) beep_phys, REG_DMA_ADDR);
...
#define ath79_reset_rr(reg) (rb400_readl(RESET_REG(reg)))
#define ath79_pll_rr(reg) (rb400_readl(PLL_REG(reg)))
...
static void maybe_ioremap_reset_base(void) {
	if (reset_base == NULL) {
...
static int __init rb400_postsetup(void)
{
	...
	maybe_ioremap_reset_base();
	pll_base = ioremap(PLL_BASE, PAGE_SIZE);
...
static int is_rev(unsigned id) {
	...
	unsigned rev;
	maybe_ioremap_reset_base();
	rev = rb400_readl(RESET_REG(RST_VERSION)) & 0xfff0;
	return rev == id;
    }
    else {
	return 0;
