...
{
    int i;
    if (!free->userspace_addr) return;

    // XXX - assuming this is only called once for guest phys memslot

    for (i = 0; i < free->npages; i++) {
        unsigned long gpa = free->base_gfn + i * PAGE_SIZE;

        pte_t *pte = get_vpgd_pte(kvm, gpa);
...
static int handle_mmio(struct kvm_vcpu *vcpu)
{
	...
	set_bit(irq_reschedule_event, &ipi_vcpu->arch.ipi_events);
...
static int kvm_emulate_hv_close_all_devices(struct kvm_vcpu *vcpu)
{
	/* We do not manipulate HW in guest so far so we do nothing here. */
	vcpu->arch.regs.regs[0] = HV_OK;

	return 1;
}

static int kvm_emulate_hv_send_nmi(struct kvm_vcpu *vcpu)
{
	HV_Coord vtarget = *(HV_Coord *) &vcpu->arch.regs.regs[0];
	unsigned long info = (unsigned long) vcpu->arch.regs.regs[1];
	uint64_t flags = (uint64_t) vcpu->arch.regs.regs[2];
	HV_NMI_Info *retval = (HV_NMI_Info *) &vcpu->arch.regs.regs[0];
	struct kvm_vcpu *vcpui;
	int vcpu_id;

	vcpu_id = vtarget.x;
	if (vtarget.y != 0 || vcpu_id < 0 ||
	    vcpu_id >= atomic_read(&vcpu->kvm->online_vcpus) ||
	...
	vcpui = kvm_get_vcpu(vcpu->kvm, vcpu_id);

	mutex_lock(&vcpui->arch.nmi_lock);

	vcpui->arch.nmi_flags = flags;
	smp_wmb();
	vcpui->arch.nmi_info = info;
	kvm_vcpu_kick(vcpui);

	wait_for_completion(&vcpui->arch.nmi_comp);

	memcpy(retval, &vcpui->arch.nmi_ret, sizeof(HV_NMI_Info));

	mutex_unlock(&vcpui->arch.nmi_lock);
...
/*
 * FIXME: Using the standard version makes the guest boot up very very
 * slowly.  Presumably it has to do with not setting TIF_VIRT_EXIT.
 */
...
{
	/* NMI interrupts */
	unsigned long nmi_info;
	uint64_t nmi_flags;
	HV_NMI_Info *nmi_retval;

	nmi_info = __insn_exch(&vcpu->arch.nmi_info, 0);
	if (nmi_info != 0) {
		nmi_flags = vcpu->arch.nmi_flags;
		nmi_retval = &vcpu->arch.nmi_ret;

		if ((vcpu->arch.regs.ex1 ==
		     (SPR_EX_CONTEXT_1_1__ICS_MASK | GUEST_PL)) &&
		    !(nmi_flags & HV_NMI_FLAG_FORCE)) {
			nmi_retval->result = HV_NMI_RESULT_FAIL_ICS;
			nmi_retval->pc = vcpu->arch.regs.pc;
		} else {
			nmi_retval->result = HV_NMI_RESULT_OK;
			nmi_retval->pc = vcpu->arch.regs.pc;

			vcpu->arch.sregs.EX_CONTEXT_1_0 = vcpu->arch.regs.pc;
			vcpu->arch.sregs.EX_CONTEXT_1_1 = vcpu->arch.regs.ex1;
			vcpu->arch.sregs.SYSTEM_SAVE_1_2 = nmi_info;
			vcpu->arch.regs.pc =
				vcpu->arch.sregs.INTERRUPT_VECTOR_BASE_1 +
				(INT_NMI_DWNCL << 8);
			vcpu->arch.regs.ex1 =
				SPR_EX_CONTEXT_1_1__ICS_MASK | GUEST_PL;
		}

		complete(&vcpu->arch.nmi_comp);
	}
...
int kvm_arch_vcpu_init(struct kvm_vcpu *vcpu)
{
	...
	mutex_init(&vcpu->arch.nmi_lock);
	init_completion(&vcpu->arch.nmi_comp);
