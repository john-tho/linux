...
{
	struct profile_time *p = this_cpu_ptr(&profile_time);
	u64 now = get_cycles();
	...
	if (too_much_oprofiling(p->interrupt_cycles, total_cycles)) {
		...
		this_cpu_write(op_counters_enabled, 0);
		mask_pmc_interrupts();
		return -EINVAL;
	}
	return 0;
...
int op_handle_perf_interrupt(struct pt_regs *regs, int fault_num)
{
	...
	if (!this_cpu_read(op_counters_enabled))
		return -EINVAL;

	if (status & 1) {
		oprofile_add_sample(regs, 0);
		__insn_mtspr(SPR_PERF_COUNT_0,
			     this_cpu_read(ctr[0].reset_value));
	}
	if (status & 2) {
		oprofile_add_sample(regs, 1);
		__insn_mtspr(SPR_PERF_COUNT_1,
			     this_cpu_read(ctr[1].reset_value));
	}
	...
	if (status & 4) {
		...
		__insn_mtspr(SPR_AUX_PERF_COUNT_0,
			     this_cpu_read(ctr[2].reset_value));
	}
	if (status & 8) {
		oprofile_add_sample(regs, 3);
		__insn_mtspr(SPR_AUX_PERF_COUNT_1,
			     this_cpu_read(ctr[3].reset_value));
	}
#endif
...
static void op_tile_cpu_start(void *dummy)
{
	...
	__insn_mtspr(SPR_PERF_COUNT_0, this_cpu_read(ctr[0].reset_value));
	__insn_mtspr(SPR_PERF_COUNT_1, this_cpu_read(ctr[1].reset_value));
	__insn_mtspr(SPR_PERF_COUNT_CTL,
		     (this_cpu_read(ctr[1].perf_count_ctl) << 16) |
		     this_cpu_read(ctr[0].perf_count_ctl));
	...
	__insn_mtspr(SPR_AUX_PERF_COUNT_0, this_cpu_read(ctr[2].reset_value));
	__insn_mtspr(SPR_AUX_PERF_COUNT_1, this_cpu_read(ctr[3].reset_value));
	__insn_mtspr(SPR_AUX_PERF_COUNT_CTL,
		     (this_cpu_read(ctr[3].perf_count_ctl) << 16) |
		     this_cpu_read(ctr[2].perf_count_ctl));
#endif

	this_cpu_write(op_counters_enabled, 1);

	/*
	 * Save away the time that we enabled profiling, and clear
	 * the count of interrupt cycles seen to date.
	 */
	this_cpu_write(profile_time.profile_start_time, get_cycles());
	this_cpu_write(profile_time.interrupt_cycles, 0);
...
static void op_tile_cpu_setup(void *dummy)
{
	...
	for (i = 0; i < TILE_NUM_PERF_COUNTERS; i++) {
		...
		this_cpu_write(ctr[i].perf_count_ctl, perf_count_ctl);
		this_cpu_write(ctr[i].reset_value, reset_val);
	}
...
static int op_tile_create_files(struct dentry *root)
{
	int i;
