...
{
	return memblock_alloc_low(L2_KERNEL_PGTABLE_SIZE, HV_PAGE_TABLE_ALIGN);
}
...
static void init_prealloc_ptes(int node, int pages)
{
	...
	if (pages) {
		...
		l2_ptes[node] = memblock_alloc_low(pages * sizeof(pte_t),
						   HV_PAGE_TABLE_ALIGN);
	}
}
...
{
	return memblock_alloc_low(L1_KERNEL_PGTABLE_SIZE, HV_PAGE_TABLE_ALIGN);
}
...
#else
int __ro_after_init kstack_hash;      /* kernel stacks */
...
#else
int __ro_after_init kdata_huge = 1;   /* kernel huge pages */
...
static int __init setup_kdata(char *str)
{
	...
	kdata_arg_seen = 1;
	pr_info("kdata: using caching neighborhood %*pbl\n", cpumask_pr_args(&kdata_mask));
	return 0;
}
...
static void __init kernel_physical_mapping_init(pgd_t *pgd_base)
{
	 ...
	 * Flush the next line too, to handle potential prefetcher issues.
	 */
	__insn_finv(&swapper_pgprot);
	__insn_finv((char *)&swapper_pgprot + CHIP_L2_LINE_SIZE());
}
...
int devmem_is_allowed(unsigned long pagenr)
{
	...
	return pagenr < kaddr_to_pfn(_end) &&
		...
		!(pagenr >= kaddr_to_pfn(_sinittext - text_delta) &&
		  pagenr <= kaddr_to_pfn(_einittext - text_delta - 1));
}
...
void __init set_page_homes(void)
{
	 ...
	 */
	for (address = PAGE_OFFSET + mem_start; address < (unsigned long) _end;
	     address += PAGE_SIZE) {
		enum { CODE_DELTA = MEM_SV_START - PAGE_OFFSET };
