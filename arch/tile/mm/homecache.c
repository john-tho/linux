...
{
	unsigned long *frames = (*this_cpu_ptr(&he_context)).frames;
	unsigned long top = (stack_pointer & -THREAD_SIZE) + THREAD_SIZE -
		(STACK_TOP_DELTA + KSTK_PTREGS_GAP + sizeof(struct pt_regs));
					 ...
					 pte_t **ptepp)
{
	...
	/* Use __set_pte() to allow overriding the home. */
...
void homecache_finv_map_page(struct page *page, int home)
{
	...
	if (home == PAGE_HOME_UNCACHED)
		return;
	BUG_ON(homecache_incoherent_home(home));
...
static void homecache_finv_page_internal(struct page *page, int force_map)
{
	...
	if (home == PAGE_HOME_UNCACHED)
		return;
	if (homecache_incoherent_home(home)) {
		int cpu;
...
#endif
	} else if (force_map || PageHighMem(page)) {
...
/*
 * Free all the pages on a list back to the buddy allocator.
 *
 * Before we free them, we flush them from the cache, reset their homes
 * to the default, and reset their lowmem PTE (if any) to the new home.
 * This amortizes the cost of later global TLB flushes for each page
 * since they will likely be allocated back with different homing requested.
 */
static void homecache_free_page_list(struct list_head *work_list)
{
	unsigned long low_kaddr = ~0UL, high_kaddr = 0;
	struct page *page, *tmp;

	list_for_each_entry(page, work_list, lru) {
		if (page_home(page) == default_page_home())
			continue;
	...
	++current->thread.homecache_is_unsequestering;
...
/* Information about coherent pages to be freed. */
struct coherent_pages {
	struct work_struct work;
	struct zone *zone;
};

/*
 * Free half of the coherent pages in a zone. We do this in a fair way
 * across all homes.
 */
static void do_coherent_free_pages(struct zone *zone)
{
	int i;
	unsigned long flags;
	LIST_HEAD(work_list);
	...
	for (i = 0; i < NR_ZONE_HOMECACHE_LISTS; i++) {
		struct homecache_list *homecache_list;
		unsigned long count_to_free;
		struct page *page, *tmp;

		if (i < NR_CPUS && !cpu_cacheable(i))
			continue;

		homecache_list = &zone->homecache_list[i];
		if (list_empty(&homecache_list->list))
			continue;

		count_to_free = zone->homecache_list[i].count / 2;
		zone->homecache_list[i].count -= count_to_free;
		zone->homecache_count -= count_to_free;

		list_for_each_entry_safe(page, tmp, &homecache_list->list,
					 lru) {
			if (!count_to_free--)
				break;

			list_del(&page->lru);
			list_add(&page->lru, &work_list);
		}
	}
	...
	if (!list_empty(&work_list))
		homecache_free_page_list(&work_list);
}

/* Kick off free of coherent pages via the work queue. */
static void coherent_free_pages(struct work_struct *work)
{
	struct coherent_pages *cpage =
		container_of(work, struct coherent_pages, work);
	do_coherent_free_pages(cpage->zone);
	kfree(cpage);
}

/*
 * Free coherent pages via the unbound work queue. We use the
 * unbound queue since we don't care where the free runs, but more
 * importantly, we don't want to be running these in parallel since we
 * would be hammering away on the memory subsystem pretty hard.
 */
static void queue_coherent_pages(struct zone *zone)
{
	struct coherent_pages *cpage =
		kmalloc(sizeof(struct coherent_pages), __GFP_NOWARN);

	if (cpage == NULL) {
		do_coherent_free_pages(zone);
		return;
	}

	INIT_WORK(&cpage->work, coherent_free_pages);
	cpage->zone = zone;
	queue_work(system_unbound_wq, &cpage->work);
}
...
int homecache_check_free_page(struct page *page, int order)
{
	...
	if (current->thread.homecache_is_unsequestering)
		return 0;
	...
	/*
	 * We are about to add something to the homecache free lists,
	 * so take a moment to see if we need to trim the lists a bit first.
	 * A high watermark percentage of pages from a given zone being in
	 * the zone's homecache freelists triggers the page-free path to
	 * extract half of the freelists' pages and return them to the buddy
	 * allocator.
	 */
	if (zone->homecache_count >= zone->present_pages / 8)
		queue_coherent_pages(zone);
 ...
 */
static struct homecache_list *homecache_zone_list(struct zone *zone, int home)
{
	if (home >= 0)
	...
	panic("Attempting to put a page with home %d on homecache free list\n",
	      home);
}
				       ...
				       struct list_head *list)
{
	...
	dump_page(page, "bad page flags when sequestering");
	dump_stack();
}

/* Information about an incoherent page to be freed. */
struct incoherent_page {
	struct work_struct work;
	struct page *page;
	int pages;  /* number of small pages that this page is made up of */
};

/*
 * Free an incoherent page.  We need to do finv-based invalidating
 * very aggressively, but once that's done, we can simply reset the
 * page home back to what is used to be and pass it to the heap, since the
 * kernel TLB entries don't need to be updated.  There's no reason to
 * batch up freeing incoherent pages since there's no per-batch overhead.
 * That said, for huge pages we do batch them up, since we might as
 * well amortize the cost of allocating incoherent_page structs.
 */
static void do_incoherent_free_page(struct page *page, int pages)
{
	int i;
	for (i = 0; i < pages; ++i, ++page) {
		int home = PageHighMem(page) ? default_page_home() :
			pte_to_home(*virt_to_kpte((long)page_to_virt(page)));
		homecache_finv_page(page);
...
/* Kick off free of an incoherent page via the work queue. */
static void incoherent_free_page(struct work_struct *work)
{
	struct incoherent_page *ipage =
		container_of(work, struct incoherent_page, work);
	do_incoherent_free_page(ipage->page, ipage->pages);
	kfree(ipage);
}

/*
 * Free an incoherent page via the unbound work queue.  We use the
 * unbound queue since we don't care where the free runs, but more
 * importantly, we don't want to be running these in parallel since we
 * would be hammering away on the memory subsystem pretty hard.
 */
static void queue_incoherent_page(struct page *page, int pages)
{
	struct incoherent_page *ipage =
		kmalloc(sizeof(struct incoherent_page), __GFP_NOWARN);

	/* If we can't kmalloc, no harm; we'll just free_page more slowly. */
	if (ipage == NULL) {
		do_incoherent_free_page(page, pages);
		return;
	}

	INIT_WORK(&ipage->work, incoherent_free_page);
	ipage->page = page;
	ipage->pages = pages;
	queue_work(system_unbound_wq, &ipage->work);
}
...
void homecache_keep_free_page(struct page *page, int order)
{
	...
	if (homecache_incoherent_home(home)) {
		queue_incoherent_page(page, pages);
		return;
	}

	zone = page_zone(page);
	homecache_list = homecache_zone_list(zone, home);
	list = &homecache_list->list;
	spin_lock_irqsave(&zone->lock, flags);
	homecache_proc_sequestered_free += pages;
	for (i = 0; i < pages; ++i)
		homecache_add_to_free_list(&page[i], list);
	homecache_list->count += pages;
	zone->homecache_count += pages;
	spin_unlock_irqrestore(&zone->lock, flags);
}
...
static void check_page_home(struct page *page, int home)
{
	...
	if (!homecache_incoherent_home(home) && !PageHighMem(page)) {
...
struct page *homecache_get_cached_page(struct zone *zone, gfp_t gfp_flags)
{
	...
	if (!list_empty(list)) {
		...
		check_page_home(page, home);
		list_del(&page->lru);
		--homecache_list->count;
		--zone->homecache_count;
		goto unlock;
	}
	...
	if (require_homecache) {
		...
		do {
			...
			i = i ? i - 1 : NR_ZONE_HOMECACHE_LISTS - 1;
		...
		for (order = HUGETLB_PAGE_ORDER; order >= 0; --order) {
			...
			page = homecache_rmqueue(zone, order, migratetype, gfp_flags);
			if (page)
				break;
		}
		if (page) {
			...
			for (i = 1; i < pages; ++i) {
				homecache_list = homecache_zone_list(zone,
							   page_home(&page[i]));
				list = &homecache_list->list;
				homecache_add_to_free_list(&page[i], list);
			}
			homecache_list->count += pages - 1;
			zone->homecache_count += pages - 1;
			goto unlock;
		}
	}
	...
	for_each_online_pgdat(pgdat) {
		...
		for (i = 0; i < MAX_NR_ZONES; i++) {
			...
			if (!total_mem &&
				...
				continue;
#endif
			npages += pgdat->node_zones[i].homecache_count;
		}
	}
					 ...
					 struct list_head *work_list)
{
	...
	for (i = 0; i < NR_ZONE_HOMECACHE_LISTS; ++i) {
		...
		list_splice_init(&zone->homecache_list[i].list, work_list);
		zone->homecache_count -= zone->homecache_list[i].count;
		zone->homecache_list[i].count = 0;
	}
	spin_unlock_irqrestore(&zone->lock, flags);
}

/*
 * This routine is called from get_page_from_freelist() when we fall
 * below the watermark for the zone, and at that point we dump all the
 * pages in the freelists back into the buddy allocator.
 */
int homecache_recover_free_pages_zone(struct zone *zone)
{
	LIST_HEAD(work_list);

	/* Collect free pages from this zone. */
 ...
 *
 * Note that we always just use the default home for incoherent pages
 * when we are fixing the kernel PTEs.  That way when we later free
 * those pages, we can just reset the page_home() to the default home,
 * invalidate the data out of all the caches, and return the page to the
 * allocator without having to worry about fixing the kernel TLB entry.
 *
					...
					int finished)
{
	...
	for (i = 0; i < pages; ++i) {
		...
		pteval = pfn_pte(pfn + i, pte_set_home(PAGE_KERNEL, home));
		if (!finished) {
			if (!pte_same(*ptep, pteval)) {
				panic("Poisoning PTE @%p for PFN %#lx:"
				      " value %#llx not %#llx\n",
				      ptep, pfn + i, ptep->val, pteval.val);
			}
			   ...
			   struct vm_area_struct *vma, int writable)
{
	...
	if (!(vma->vm_flags & VM_SHARED) &&
	    vma->vm_file && mapping_writably_mapped(vma->vm_file->f_mapping))
		return;
					  ...
					  unsigned long start_va)
{
	...
	if (ms->migrating_index == 0) {
		...
		/*
		 * Construct the cpu/ASID vector to flush, based on
		 * what other threads are sharing this mm.
		 */
		ms->num_rem_asids =
			flush_tlb_mm_begin(p->mm, ms->rem_asids, true);
	}
...
void homecache_init_zone_lists(struct zone *zone)
{
	...
	for (i = 0; i < NR_ZONE_HOMECACHE_LISTS; i++) {
		INIT_LIST_HEAD(&zone->homecache_list[i].list);
		zone->homecache_list[i].count = 0;
	}
	zone->homecache_count = 0;
	zone->lru_homecache_list = 0;
}

static struct ctl_table homecache_table[] = {
	{
...
static struct ctl_table tile_table[] = {
	{
		.procname	= "homecache",
		.mode		= 0555,
