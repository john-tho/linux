...
#ifdef __tilegx__  /* NMI only available on tilegx */
	unsigned long nmi_info; /* argument of hv_send_nmi() */
	uint64_t nmi_flags; /* argument of hv_send_nmi() */
	HV_NMI_Info nmi_ret; /* Return value of hv_send_nmi() */
	struct mutex nmi_lock; /* One nmi on the vcpu each time. */
	struct completion nmi_comp;
#endif
};
