...
/* List of free homecacheable pages. */
struct homecache_list {
	struct list_head list;
	/* Number of homecacheable pages in this list. */
	unsigned long count;
};
...
static inline bool homecache_incoherent_home(int home)
{
 ...
 * additional lists for uncached and hash-for-home.
 */
#define ZONE_HOMECACHE_UNCACHED_INDEX (NR_CPUS)
#define ZONE_HOMECACHE_HFH_INDEX (NR_CPUS + 1)
#define NR_ZONE_HOMECACHE_LISTS (NR_CPUS + 2)
...
/* Manipulate the flags for mmap(). */
extern int arch_vm_area_flags(unsigned long prot, unsigned long flags);
