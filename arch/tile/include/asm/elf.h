...
/*
 * Re-install the context for the m32 process to set the HV_CTX_32BIT
 * context flag when set the personality, so that hypervisor can do a
 * mirror flush in the top 2GB of VA space when we flush in the 2-4GB range.
 * Set a bit in the mm as well to make it easy for flush_tlb_range()
 * etc to determine whether a given flush is for a 32-bit process.
 */
#define COMPAT_SET_PERSONALITY(ex) \
do { \
	set_personality(PER_LINUX | (current->personality & (~PER_MASK))); \
	current_thread_info()->status |= TS_COMPAT; \
	current->mm->context.is_32bit = true; \
	compat_reset_context(); \
} while (0)
