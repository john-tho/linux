...
{
	__this_cpu_inc(irq_stat.irq_call_count);
	switch (tag) {
...
struct ipi_flush {
	unsigned long start;
	unsigned long end;
	int nc;   /* non-coherent in L2 cache */
};
...
static void ipi_flush_icache_range(void *info)
{
	...
	if (flush->nc)
		__finv_buffer((void *)(flush->start),
			      flush->end - flush->start);
	__flush_icache_range(flush->start, flush->end);
}

void flush_icache_range(unsigned long start, unsigned long end)
{
	/* Assume any run of code is either all non-coherent or coherent. */
	int nc = hv_pte_get_nc(*virt_to_kpte(start));

	/* If invoked with irqs disabled, we can not issue IPIs. */
	if (irqs_disabled()) {
		int flags = HV_FLUSH_EVICT_L1I;
		if (nc) {
			/*
			 * We could compute the PA (possibly multiple
			 * chunks if the start/end range crosses
			 * pages) to flush the L2 with more precision,
			 * but note that the Linux flush_remote()
			 * wrapper doesn't support sub-page
			 * granularity, and this is the backup path
			 * for when interrupts are disabled anyway, so
			 * we assume it doesn't matter too much.
			 */
			flags |= HV_FLUSH_EVICT_L2;
		}
		flush_remote(0, flags, cpu_online_mask, 0, 0, 0,
			     NULL, NULL, 0);
	} else {
		struct ipi_flush flush = { start, end, nc };
		preempt_disable();
...
void __init ipi_init(void)
{
	...
	irq_reschedule = irq_alloc_hwirq(-1);
...
static void __smp_send_reschedule(int cpu)
{
	 ...
	 */
	unsigned long *addr =
		&((unsigned long __force *)ipi_mappings[cpu])
		[irq_reschedule_event];
