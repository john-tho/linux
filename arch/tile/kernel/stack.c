			     ...
			     struct task_struct *t, struct pt_regs *regs)
{
	...
	if (regs == NULL) {
		...
		sp = t->thread.ksp;
		/*
		 * Load the fp from the stack. The offset depends on the
		 * stack push/pop code in __switch_to. If there is any
		 * modification in __switch_to, the following line possibly
		 * needs a fix corrrespondingly.
		 */
		r52 = *(unsigned long *)(sp + CALLEE_SAVED_REGS_COUNT * 8);
	} else {
		pc = regs->pc;
		lr = regs->lr;
		sp = regs->sp;
...
void tile_show_stack(struct KBacktraceIterator *kbt)
{
	...
	for (; !KBacktraceIterator_end(kbt); KBacktraceIterator_next(kbt)) {
		...
		pr_err("  frame %d: %pS (sp 0x%lx)\n",
		       i++, (void *)address, (unsigned long)(kbt->it.sp));
