...
/*
 * If we enter cpu_idle with our stack still set to be the initial
 * stack, we switch to a new stack page now (and free the initial
 * stack back to the heap).  This allows the boot cpu's idle process
 * to run with a stack that has proper homecaching, without requiring
 * us to potentially shatter the initial hugepage that
 * init_thread_union is on.
 */
void arch_cpu_idle_prepare(void)
{
	if (current_thread_info() == &init_thread_union.thread_info) {
		struct thread_info *ti =
		    (struct thread_info *) alloc_thread_stack_node(current, -1);
...
#ifdef CONFIG_DATAPLANE
/*
 * On dataplane cores we always prevent waking ksoftirqd, since it can
 * end up causing a dataplane task to have trouble quiescing in the
 * presence of another runnable task.  We check on return to userspace
 * to make sure we have no pending softirqs, so here we also check on
 * return to idle, since a dataplane core will always go back to one
 * or the other.  Note interrupts are disabled here.
 */
void arch_cpu_idle_enter(void)
{
	if (cpumask_test_cpu(smp_processor_id(), &dataplane_map) &&
...
unsigned long *alloc_thread_stack_node(struct task_struct *task, int node)
{
	 ...
	 */
	int home = kstack_hash ? PAGE_HOME_HASH : raw_smp_processor_id();
	struct page *page =
		homecache_alloc_pages_node(node, THREADINFO_GFP,
					   THREAD_SIZE_ORDER, home);

	return page ? page_address(page) : NULL;
}

void __free_thread_stack(unsigned long *stack)
{
	arch_release_thread_stack(stack);
	free_pages((unsigned long) stack, THREAD_SIZE_ORDER);
}

void free_thread_stack(struct task_struct *tsk)
{
	__free_thread_stack((unsigned long *) tsk->stack);
}
...
/* Capture the right startup value for any context-switched state. */
void init_arch_state(struct thread_struct *t)
{
...
#ifdef __tilegx__
	/* Unmap current thread's unalign jit page. */
	if (info->unalign_jit_base) {
		addr = (unsigned long)info->unalign_jit_base;
		size = PAGE_SIZE;
	}
#else
	/*
	 * Unmap buffer(64 bytes) in struct single_step_state for
	 * current thread.
	 */
	if (info->step_state && info->step_state->buffer) {
		addr = (unsigned long)info->step_state->buffer;
		size = 64;
	}
#endif
	if (mm && addr) {
		...
		ret = do_munmap(mm, addr, size, NULL);
		up_write(&mm->mmap_sem);

		if (ret)
			pr_err("%s unmap unalign mem 0x%lx, %d\n",
			       current->comm, addr, ret);
	}
