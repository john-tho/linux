...
static struct hardwall_type hardwall_types[] = {
	...
	[HARDWALL_UDN] = {
		...
		.enabled = 1,
		.name = "udn",
		.list = LIST_HEAD_INIT(hardwall_types[HARDWALL_UDN].list),
		.lock =
	...
	[HARDWALL_IPI] = {
		...
		.enabled = 1,
		.name = "ipi",
		.list = LIST_HEAD_INIT(hardwall_types[HARDWALL_IPI].list),
		.lock =
...
/*
 * We use this macro, rather than referencing hwt->is_udn directly, so that
 * when we don't have the UDN, the compiler can optimize out the UDN code.
 */
#ifdef HAVE_UDN
#define hardwall_is_udn(hwt) ((hwt)->is_udn)
#else
#define hardwall_is_udn(hwt) 0
#endif
...
struct hardwall_info {
	...
	atomic_t udn_pending_count;        /* cores in phase 1 of drain */
#endif
};
...
static int __init noudn(char *str)
{
	...
	hardwall_types[HARDWALL_UDN].enabled = 0;
	return 0;
}
...
static int __init noipi(char *str)
{
	...
	hardwall_types[HARDWALL_IPI].enabled = 0;
	return 0;
}
...
/* Bit field values to mask together for writes to SPR_UDN_DIRECTION_PROTECT */
...
static int hardwall_activate(struct hardwall_info *info)
{
	...
	BUG_ON(cpumask_first(&p->cpus_mask) != cpu);
...
static void stop_udn_switch(void *arg)
{
#if !CHIP_HAS_REV1_XDN()
	 ...
	 */
	unsigned long protect = __insn_mfspr(SPR_UDN_DIRECTION_PROTECT);
	__insn_mtspr(SPR_UDN_DIRECTION_PROTECT, (protect | C_PROTECT) << 5);
#endif
}

static void empty_udn_demuxes(struct hardwall_type *hwt)
{
...
static void drain_udn_switch(void *arg)
{
	 ...
	 */
	int pending = __insn_mfspr(SPR_UDN_PENDING);
	while (pending--) {
		empty_udn_demuxes(hwt);
		__tile_udn_send(0);
	}
	atomic_dec(&info->udn_pending_count);
	while (atomic_read(&info->udn_pending_count))
		empty_udn_demuxes(hwt);
#else
	int i;
...
static void reset_udn_network_state(struct hardwall_type *hwt)
{
	if (!hwt->enabled)
		return;
...
{
#ifdef HAVE_UDN
	reset_udn_network_state(&hardwall_types[HARDWALL_UDN]);
#endif
}

/* Restart a UDN switch after draining. */
static void restart_udn_switch(void *arg)
{
...
static void hardwall_destroy(struct hardwall_info *info)
{
	...
	if (hardwall_is_udn(hwt)) {
		...
		on_each_cpu_mask(&info->cpumask, stop_udn_switch, hwt, 1);

		/* Drain the network. */
#if CHIP_HAS_REV1_XDN()
		atomic_set(&info->udn_pending_count,
			   cpumask_weight(&info->cpumask));
		on_each_cpu_mask(&info->cpumask, drain_udn_switch, info, 0);
#else
		on_each_cpu_mask(&info->cpumask, drain_udn_switch, info, 1);
#endif

		/* Restart switch and disable firewall. */
		on_each_cpu_mask(&info->cpumask, restart_udn_switch, hwt, 1);
	}
...
static const struct proc_ops hardwall_proc_fops = {
	.proc_open	= hardwall_proc_open,
	.proc_read	= seq_read,
