		    ...
		    int align_ctl)
{
	...
	if (EX1_PL(regs->ex1) != USER_PL) {
		...
		if (unexpected) {
			...
			unsigned char __user *ptr =
				(unsigned char __user *)regs->regs[ra];
			if (load_n_store) {
				/* handle get_user(x, ptr) */
				if (is_compat_task())
					ptr = compat_ptr_adjust(
							(void __user *)ptr);
	...
	if ((align_ctl == 0) || unexpected) {
		...
		force_sig_fault(SIGBUS, BUS_ADRALN, addr, 0);
		return;
	}
	...
	if (!unexpected) {
		...
		if (status) {
			...
			trace_unhandled_signal("segfault in unalign fixup",
					       ...
					       (unsigned long) addr,
					       SIGSEGV);
			force_sig_fault(SIGSEGV, SEGV_MAPERR, addr, 0);
			return;
		}
		...
		__insn_mtspr(SPR_EX_CONTEXT_0_1, regs->ex1);
...
void do_unaligned(struct pt_regs *regs, int vecnum)
{
	...
	/*
	 * Fault came from user. If stack is not aligned or align_ctl < 0,
	 * trigger a SIGBUS.
	 */
	if ((regs->sp & 0x7) || (align_ctl < 0)) {
		current->thread.trap_nr = BUS_ADRALN;

		if (unaligned_printk)
			pr_info("Unalign fixup: %d %llx @%llx %llx\n",
				(int)unaligned_fixup,
				(unsigned long long)regs->ex1,
				(unsigned long long)regs->pc,
		...
		force_sig_fault(SIGBUS, BUS_ADRALN, 0, 0);
		return;
	}
	...
	if (is_compat_task())
		pc = compat_ptr_adjust(pc);
	if (get_user(bundle, pc) != 0) {
		/* Probably never be here since pc is valid user address.*/
		current->thread.trap_nr = SEGV_MAPERR;
		pr_err("Couldn't read instruction at %p trying to step\n", pc);
		trace_unhandled_signal("segfault in unalign fixup", regs,
				       (unsigned long) pc, SIGSEGV);
		force_sig_fault(SIGSEGV, SEGV_MAPERR, (void __user *) pc, 0);
		return;
	}
