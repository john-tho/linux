...
static unsigned long tile_mmap(unsigned long addr, unsigned long len,
			       ...
			       unsigned long fd, unsigned long off, int shift)
{
	...
	/*
	 * The MAP_HUGETLB bits which allow selecting huge page size
	 * are not supported on Tile Linux when homecaching is enabled,
	 * since they conflict with the homecache flags, so we clear
	 * them explicitly after reading them in arch_vm_area_flags().
	 * NOTE: we might prefer to always clear them regardless of
	 * CONFIG_HOMECACHE to make the kernel API consistent.
	 */
	if (arch_vm_area_flags(prot, flags))
		return -EINVAL;
	flags &= ~(MAP_HUGE_MASK << MAP_HUGE_SHIFT);
#endif
	ret = sys_mmap_pgoff(addr, len, prot, flags, fd, off >> shift);
#ifdef CONFIG_HOMECACHE
...
{
	return tile_mmap(addr, len, prot, flags, fd, off_4k, PAGE_SHIFT - 12);
}
#endif
...
{
	return tile_mmap(addr, len, prot, flags, fd, offset, PAGE_SHIFT);
}
#endif
...
static int __ro_after_init utext_hash = 1;
static int __ro_after_init udata_hash = 1;
static int __ro_after_init ustack_hash = 0;
int __ro_after_init uheap_hash = 1;
...
int arch_vm_area_flags(unsigned long protflags, unsigned long flags)
{
	pgprot_t prot = __pgprot(0);
	pid_t pid = 0;

	/* By default, no homecaching overrides will be used. */
	current->thread.homecache_prot = 0;
	...
	current->thread.homecache_prot = prot.val;
	current->thread.homecache_pid = pid;
