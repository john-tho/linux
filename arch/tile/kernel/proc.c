...
#define SEQ_PROC_ENTRY(name)						\
	...
	static const struct proc_ops proc_tile_##name##_fops = { \
		.proc_open	= proc_tile_##name##_open,		\
		.proc_read	= seq_read,				\
	...
	{								\
		proc_create(#name, 0444, root, &proc_tile_##name##_fops); \
	}

#ifdef __tilegx__
/* Allow tagging deprecated /proc files. */
static void check_deprecated(int *warned, const char *old, const char *new)
{
	if (!*warned) {
		printk("process '%s' is using deprecated file /proc/tile/%s;"
		       " use %s instead.\n", current->comm, old, new);
		*warned = 1;
	}
}
#endif
...
static int proc_tile_environment_show(struct seq_file *sf, void *v)
{
	...
	check_deprecated(&warned, "environment",
			 "/sys/class/hwmon/hwmon0/device/temp[12]_input");
#endif
	if (cpu_temp < 0)
...
static const struct proc_ops proc_tile_interrupts_fops = {
	.proc_open	= proc_tile_interrupts_open,
	.proc_read	= seq_read,
	.proc_write	= proc_tile_interrupts_write,
	.proc_lseek	= seq_lseek,
	.proc_release	= single_release,
};
...
{
	proc_create("interrupts", 0644, root, &proc_tile_interrupts_fops);
}
...
{
	proc_create("scan_pci", 0644, root, &proc_tile_scan_pci_fops);
}
#else
...
static struct ctl_table hash_default_table[] = {
	{
...
struct ctl_table userspace_perf_counters_table[] = {
	{
