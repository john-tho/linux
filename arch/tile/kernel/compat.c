...
/*
 * We need to set the "compat" bit in the installed page table, which
 * doesn't happen at the initial fork/exec because we haven't yet
 * set up the new address space with the TS_COMPAT bit.
 */
void compat_reset_context(void)
{
	struct mm_struct *mm = current->mm;
	install_page_table(mm->pgd, __this_cpu_read(current_asid), 1);
}
