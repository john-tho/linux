...
static int __init setup_mem(char *p)
{
	unsigned long size;
	char *endp;
	...
	if (*endp == '@') {
#ifdef CONFIG_HAVE_KDUMP
		unsigned long start = memparse(endp + 1, NULL);
		mem_start = HPAGE_UP(start);
		size -= (mem_start - start);
#else
		pr_err("@ in 'mem=' is not supported on this platform.\n");
#endif
	}

	maxmem_pfn = HPAGE_DOWN(size) >> PAGE_SHIFT;
...
static void __init setup_memory(void)
{
	...
	for (i = 0; ; ++i) {
		...
		if (mem_start >= range.start &&
		    mem_start < (range.start + range.size)) {
			range.size -= (mem_start - range.start);
			range.start = mem_start;
		}
...
static void __init reserve_crashkernel(void)
{
	...
	unsigned long crash_start_pfn, crash_end_pfn;
	unsigned long total_pages;
	int i, ret;

	total_pages = 0;
	for (i = 0; i < MAX_NUMNODES; ++i)
		total_pages += node_end_pfn[i] - node_start_pfn[i];

	ret = parse_crashkernel(boot_command_line, PFN_PHYS(total_pages),
			       &crash_size, &crash_base);
	if (ret)
		return;

	/* FIXME: Allow determining crash_base automatically. */

	crash_end = crash_base + crash_size - 1;

	/* Should be huge page size aligned. */
	crash_base = HPAGE_UP(crash_base);
	crash_start_pfn = crash_base >> PAGE_SHIFT;
	crash_end = HPAGE_DOWN(crash_end);
	crash_end_pfn = crash_end >> PAGE_SHIFT;
	crash_size = crash_end - crash_base;

	for (i = 0; i < MAX_NUMNODES; ++i) {
		if (crash_start_pfn >= node_start_pfn[i] &&
		    crash_end_pfn <= node_end_pfn[i])
			break;
	}

	if (i == MAX_NUMNODES) {
		printk(KERN_WARNING "crashkernel reservation failed - "
		       "illegal range or node memory ranges are crossed.\n");
		return;
	}

	ret = memblock_reserve(crash_base, crash_size, BOOTMEM_EXCLUSIVE);
	if (ret) {
		printk(KERN_WARNING "crashkernel reservation failed - "
		       "memory is in use.\n");
		return;
	...
	printk(KERN_INFO "Reserving %ldMB of memory at %ldMB "
	       ...
	       (unsigned long)pages_to_mb(total_pages));
...
#else
static void inline reserve_crashkernel(void) { }
#endif
...
static inline unsigned long alloc_bootmem_pfn(int nid, phys_addr_t size, phys_addr_t goal)
{
	void *kva = memblock_alloc_try_nid(size, PAGE_SIZE, goal,
					   MEMBLOCK_ALLOC_ACCESSIBLE, nid);
	return kaddr_to_pfn(kva);
}
...
static void __init setup_bootmem_allocator_node(int i)
{
	...
	memblock_add(PFN_PHYS(start), PFN_PHYS(end - start));
...
static void __init setup_bootmem_allocator(void)
{
	...
	for (i = 0; i < memmap_nr; ++i) {
		...
		memblock_reserve(m->addr, m->size);
	}

#ifdef CONFIG_BLK_DEV_INITRD
	if (initrd_start) {
		/* Make sure the initrd memory region is not modified. */
	    if (memblock_reserve(initrd_start, initrd_end - initrd_start)) {
			pr_crit("Some initrd memory has been "
				"used. Disabling it.\n");
			initrd_start = 0;
			initrd_end = 0;
		} else {
...
static void __init zone_sizes_init(void)
{
	...
	for_each_online_node(i) {
		...
		if (i != 0 && node_isset(i, isolnodes)) {
			...
			if (node_percpu[i])
				node_percpu_pfn[i] =
				    alloc_bootmem_pfn(i, node_percpu[i], goal);
		} else {
...
static void __init load_hv_initrd(void)
{
	...
	initrd = memblock_alloc(stat.size, PAGE_SIZE);
	...
	if (rc != stat.size) {
		...
		memblock_free((unsigned long) initrd, stat.size);
		return;
	}
...
{
	memblock_free_late(__pa(begin), end - begin);
}
...
static void __init validate_hv(void)
{
	...
	BUG_ON(min_asid > ASID_MASK);
	if (max_asid > ASID_MASK) {
		pr_err("Warning: not using full range of available ASIDs"
		       " (max %d, ASID_MASK %#x)\n", max_asid, ASID_MASK);
		max_asid = ASID_MASK;
	}
	set_init_asid(min_asid);
...
void __init setup_arch(char **cmdline_p)
{
	...
	*cmdline_p = boot_command_line;
	...
	setup_clock();
	load_hv_initrd();
	init_arch_state(&current->thread);
}
...
static void __init pcpu_fc_populate_pte(unsigned long addr)
{
	...
	if (pmd_present(*pmd)) {
		...
		pte = memblock_alloc_from(L2_KERNEL_PGTABLE_SIZE,
