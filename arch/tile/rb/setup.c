...
static unsigned long add_sysfs_gpio_state_file(struct class *hw_info,
					       const char *filename,
					       struct class_attribute_gpio *info,
					       int gpio_num) {
...
unsigned long add_psu_state(unsigned gpio_numA, unsigned gpio_numB) {
    static struct class_attribute_gpio psu1;
    static struct class_attribute_gpio psu2;

    unsigned long pins = 0;
    struct class *hw_info = class_create(THIS_MODULE, "hw_info");
    pins |= add_sysfs_gpio_state_file(hw_info, "psu1_state", &psu1, gpio_numA);
    pins |= add_sysfs_gpio_state_file(hw_info, "psu2_state", &psu2, gpio_numB);
    return pins;
}
...
static void export_specific_mode_button_gpio(int num) {
    extern int gpiod_configure_flags(struct gpio_desc *desc, const char *con_id,
				     unsigned long lflags, enum gpiod_flags dflags);
    
    static const char *gpio_names[64];
    gpio_names[num] = "mode-button";
    gpio_to_chip(num)->names = gpio_names;
    gpio_request(num, "mode-button");
    gpiod_configure_flags(gpio_to_desc(num), "", GPIO_ACTIVE_LOW, 0);
...
static int ccr_platform_init_r2(void) {
    unsigned long in_pins = 0, out_pins = 0, od_pins = 0, all_pins = 0;
    signed long mode_button = -1;
    if (is_name_prefix("CCR1016-12S-1S+")) {
	_fan_sense_gpios = 0;
	_fan_sense_count = 0;
	in_pins = GPI_BUTTON
	    | GPI_CCR1036R2_MODE_BUTTON
	    | GPI_STM8_FAN_IRQ;
	out_pins = GPO_BEEP
	    | GPO_USER_LED
	    | GPO_LCD_LED
	    ...
	    | GPO_FAULT_LED
	    | GPO_STM8_FAN_RST;
	mode_button = CCR1036R2_MODE_BUTTON;
    }
    else if (is_name_prefix("CCR1036-8G-2S+")) {
	_fan_sense_gpios = 0;
	_fan_sense_count = 0;
	in_pins = GPI_BUTTON
	    | GPI_CCR1036R2_MODE_BUTTON
    ...
    else if (is_name_prefix("CCR1036-12G-4S")
	  || is_name_prefix("CCR1016-12G")) {
	_fan_sense_gpios = 0;
	_fan_sense_count = 0;
	in_pins = GPI_BUTTON
	    | GPI_CCR1036R2_MODE_BUTTON
    ...
    if (mode_button >= 0) {
	export_specific_mode_button_gpio(mode_button);
    }
...
int ccr_platform_init(void) {
    ...
    if (is_board_type("ccr1036r2") || is_board_type("ccr1016r2")) {
	return ccr_platform_init_r2();
    }
