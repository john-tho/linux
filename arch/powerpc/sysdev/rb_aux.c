...
static void ioremap_from_node(const unsigned *property, unsigned **ptr) {
    ...
    *ptr = ioremap(res.start, res.end - res.start + 1);
}
...
arch_initcall(rb_declare_of_platform_devices);

static int __init rb_init_of_platform_devices(void)
{
	rb_beeper_init();
	rb_leds_init();
...
device_initcall(rb_init_of_platform_devices);
...
void __init rb_pic_init(void)
{
	...
	mpic = mpic_alloc(np, r.start, MPIC_BIG_ENDIAN, 4, 0, " OpenPIC ");
	for (i = 0; i < 80; i += 4) {
		/* 
...
void __attribute__((noreturn)) rb_restart(char *cmd)
{
	local_irq_disable();
...
void __attribute__((noreturn)) rb_power_off(void)
{
...
void add_second_serial_of_node(void)
{
	...
	np->full_name = "/soc8544@e0000000/serial@4600";	
	*(unsigned *) of_get_property(np, "reg", NULL) = 0x4600;
