...
static int on_qemu = 0;

hypercall(_vm_running, 6, void);
int vm_running(void) {
	if (on_qemu)
		return -1;
	return _vm_running();
...
static void __attribute__((noreturn)) rbvm_restart(char *command)
{
	char msg[] = "restart";
...
static void __attribute__((noreturn)) rbvm_power_off(void)
{
	char msg[] = "halt";
...
static int __init rbvm_probe(void)
{
	...
	if (of_flat_dt_is_compatible(of_get_flat_dt_root(), "fsl,qemu-e500"))
		on_qemu = 1;
...
{
	extern struct irq_domain *virq_host;
