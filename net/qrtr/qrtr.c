...
static int qrtr_getname(struct socket *sock, struct sockaddr *saddr, int peer)
{
	struct qrtr_sock *ipc = qrtr_sk(sock->sk);
...
#if 0
	case SIOCGSTAMP:
		printk("TODO: sock_get_timestamp\n");
			  ...
			  struct netlink_ext_ack *extack)
{
	...
	rc = nlmsg_parse_deprecated(nlh, sizeof(*ifm), tb, IFA_MAX, qrtr_policy, extack);
	if (rc < 0)
		return rc;
...
static int __init qrtr_proto_init(void)
{
	...
	rtnl_register_module(THIS_MODULE, PF_QIPCRTR, RTM_NEWADDR, qrtr_addr_doit, NULL, NULL);
