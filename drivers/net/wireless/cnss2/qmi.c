...
static int caldata_avail_mktk(struct device *dev, unsigned id) {
	if (id && ((id == QCA8074_DEVICE_ID) ||
			(id == QCA8074V2_DEVICE_ID) ||
			(id == QCA5018_DEVICE_ID) ||
			(id == QCA6018_DEVICE_ID))) {
		int ret;
		char path[30], *fn;
		const struct firmware *fw;
		...
		switch (id) {
		case QCA6018_DEVICE_ID:
			sprintf(path, "IPQ6018/");
			break;
		case QCA5018_DEVICE_ID:
			sprintf(path, "IPQ5018/");
			break;
		default:
			sprintf(path, "IPQ8074/");
			break;
		}
		fn = path + strlen(path);
		sprintf(fn, "bdwlan.bin%c", '\0');
		printk("Looking for \"%s\"\n", path);
		ret = request_firmware(&fw, path, dev);
		if (ret)
			return -ENOENT;
		release_firmware(fw);
		sprintf(fn, "caldata.bin%c",'\0');
		printk("Looking for \"%s\"\n", path);
		ret = request_firmware(&fw, path, dev);
		if (ret)
			return -ENOENT;
				 ...
				 u32 bdf_type)
{
	...
	ret = caldata_avail_mktk(&plat_priv->plat_dev->dev, plat_priv->device_id);
	if (ret)
		return ret;
...
int cnss_wlfw_device_info_send_sync(struct cnss_plat_data *plat_priv)
{
	...
	plat_priv->qcn6122.bar_addr_va =
				ioremap(plat_priv->qcn6122.bar_addr_pa,
