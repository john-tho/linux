			     ...
			     const struct pci_device_id *id)
{
	...
	/* Do not modify the abort mask */
	int_regs = al_udma_gen_int_regs_get(&ssm_pcie->udma);
	iofic_cfg.m2s_aborts_disable =
		al_iofic_abort_mask_read(&int_regs->secondary_iofic_ctrl,
					 AL_INT_GROUP_A);
	iofic_cfg.s2m_aborts_disable =
		al_iofic_abort_mask_read(&int_regs->secondary_iofic_ctrl,
					 AL_INT_GROUP_B);
