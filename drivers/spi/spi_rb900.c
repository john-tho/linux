...
static unsigned gpio_fn0 = 0x900;

#define     GPIN0_MISC(x)	(gpio_fn0 | (x))
#define     GPIN0_DEFAULT	GPIN0_MISC(0x8)
#define     GPIN0_TS		GPIN0_MISC(0xd)
#define     GPIN0_LCD_READ	GPIN0_MISC(0x4)
...
static unsigned int mhz[NUM_CHIPSELECT];
module_param_array(mhz, uint, NULL, 0);
...
static void rb900_spi_process_queue_locked(struct rb900_spi *rbspi,
	...
	while (!list_empty(&rbspi->queue)) {
		...
		if (m->cs_release_after_complete && cs >= 0) {
			spi_cs_terminate(rbspi, m);
			cs = -1;
		}
...
static unsigned get_ahb_clock(void) {
	...
	clk_ctrl = read32_ioremap(CPU_DDR_CLOCK_CONTROL);
	ahb_div = ((clk_ctrl >> 15) & 0x1f) + 1;
...
static int rb900_spi_probe(struct platform_device *pdev)
{
	...
	if (mhz[info->chip_select]) {
		unsigned hz = mhz[info->chip_select] * 1000000;
		unsigned div = (ahb_clock - 1) / hz;
		printk("%s (default %d, actual %d Hz)\n",
		       info->modalias,
		       info->max_speed_hz,
		       ahb_clock / (div + 1));
		info->max_speed_hz = hz;
	}
	printk("%s (%d MHz)\n",
	       info->modalias, info->max_speed_hz / 1000000);
