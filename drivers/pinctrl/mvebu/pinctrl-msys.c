...
static const struct mvebu_mpp_mode msys_mpp_modes[] = {
	MPP_MODE(0,
...
static const struct mvebu_mpp_ctrl msys_mpp_controls[] = {
...
static int msys_pinctrl_suspend(struct platform_device *pdev, pm_message_t state)
{
...
static int msys_pinctrl_resume(struct platform_device *pdev)
{
	struct mvebu_pinctrl_soc_info *soc =
		platform_get_drvdata(pdev);
	int i, nregs;
...
builtin_platform_driver(msys_pinctrl_driver);
