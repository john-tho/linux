...
static int al_read_empty_config(struct pci_bus *bus, unsigned int devfn,
				int where, int size, u32 *val)
{
	return PCIBIOS_DEVICE_NOT_FOUND;
}

static int al_write_empty_config(struct pci_bus *bus, unsigned int devfn,
				 int where, int size, u32 val)
{
	return PCIBIOS_DEVICE_NOT_FOUND;
}

static struct pci_ops al_pcie_empty_ops = {
	.read	= al_read_empty_config,
	.write	= al_write_empty_config,
};
...
static int al_pcie_probe(struct platform_device *pdev)
{
	...
	if (!al_pcie_port_check_link(pcie))
		goto skip_hw_init;
...
skip_hw_init:
	...
	bridge->ops = al_pcie_port_check_link(pcie)
	    ? &al_pcie_ops
	    : &al_pcie_empty_ops;
...
static int disable_talyn_quirk(void) {
	return of_machine_is_compatible("marvell,armada7040");
}

static void quirk_pcie_talyn(struct pci_dev *pdev)
{
	// disable msix on pci bridges with talyn behind - crashes on alpine_v2 pcie init
	if (pdev->bus->self && !disable_talyn_quirk()) {
		pdev->bus->self->msix_enabled = 1;
	}
//	printk("%s quirk talyn\n", dev_name(&pdev->dev));
}
DECLARE_PCI_FIXUP_EARLY(0x17cb,	0x1201,	quirk_pcie_talyn);
