...
static int amazon_pcie_err_probe(struct platform_device *pdev)
{
	...
	if (device_create_file(dev, &dev_attr_current_link_speed)) {
		dev_notice(dev,
			   "Failed to create sysfs for PCIe speed status\n");
	}

	if (device_create_file(dev, &dev_attr_current_link_width)) {
		dev_notice(dev,
...
static int amazon_pcie_err_remove(struct platform_device *pdev)
{
	...
	device_remove_file(pcie_err->dev, &dev_attr_current_link_speed);
	device_remove_file(pcie_err->dev, &dev_attr_current_link_width);
