...
static int qcom_pcie_phy_ver = 0;
	...
	if (pcie->qmp_pcie_phy_init_seq) {
		...
		for (i = 0; i < pcie->init_seq_len; i++) {
			if (regs[i].reg_offset == PHY_DELAY_MS)
				...
				qca_pcie_write_reg(pcie->base, regs[i].reg_offset, regs[i].val);
		}
	} else {
...
static int qca_pcie_qmp_phy_power_on(struct phy *phy)
{
	...
	do {
		if (qcom_pcie_phy_ver == 0x2102) {
			ret = ~(readl_relaxed(pcie->base + 0x1214)) & BIT(7);
		} else if (qcom_pcie_phy_ver == 0x21) {
			ret = ~(readl_relaxed(pcie->base + 0x3c)) & BIT(0);
		} else {
...
static int qca_pcie_qmp_phy_probe(struct platform_device *pdev)
{
	...
	if (!ret) {
		...
		soc_version_major = of_get_property(
	...
	if (size) {
		pcie->qmp_pcie_phy_init_seq = devm_kzalloc(dev, size, GFP_KERNEL);
		if (pcie->qmp_pcie_phy_init_seq) {
			pcie->init_seq_len = size / sizeof(*pcie->qmp_pcie_phy_init_seq) / 2;
...
//	of_property_read_u32(pdev->dev.of_node, "port_nr", &pcie->port_nr);
	...
	of_property_read_u32(dev->of_node, "qcom,pcie-phy-ver", &qcom_pcie_phy_ver);
	return 0;
}
