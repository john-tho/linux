...
static int msm_ssphy_qmp_probe(struct platform_device *pdev)
{
	...
	if (!res) {
		...
		phy->vls_clamp_reg = devm_ioremap(dev, res->start,
							resource_size(res));
	...
	if (res) {
		phy->tcsr_phy_clk_scheme_sel = devm_ioremap(dev,
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM,
							"qscratch_base");
	if (res) {
		phy->qscratch_base = devm_ioremap(dev, res->start,
							resource_size(res));
	...
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM,
							"ahb2phy_base");
	if (res) {
		phy->ahb2phy_base = devm_ioremap(dev, res->start,
							resource_size(res));
