...
extern int qcom_scm_call(struct device *dev, u32 svc_id, u32 cmd_id,
...
int qca_uni_ss_read_otp(struct usb_phy *p, uint32_t *otp_read_data)
{
	int ret;
	...
	rdip.value = dma_map_single(p->dev, otp_value,
			sizeof(uint32_t), DMA_FROM_DEVICE);

	ret = dma_mapping_error(p->dev, rdip.value);
	if (ret != 0) {
...
//		&rdip, sizeof(struct qf_read), NULL, 0);
	...
	ret = qcom_scm_call(p->dev, /*QCOM_SCM_SVC_FUSE*/0x8, USB_CALIBRATION_CMD,
		&rdip, sizeof(struct qf_read), NULL, 0);

	dma_unmap_single(p->dev, rdip.value,
		sizeof(uint32_t), DMA_FROM_DEVICE);
...
int qca_uni_ss_phy_usb_los_calibration(struct usb_phy *p, void __iomem *base)
{
	uint32_t data, otp_val = 0;

	/* Get OTP value */
	if ((qca_uni_ss_read_otp(p, &otp_val) < 0) || (!(otp_val & OTP_MASK))) {
...
static int qca_uni_ss_phy_init(struct usb_phy *x)
{
	...
	/* USB LOS Calibration */
	ret = qca_uni_ss_phy_usb_los_calibration(x, phy->base);
