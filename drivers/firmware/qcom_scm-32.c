...
int __qcom_scm_mem_protect_ipa_region(struct device *dev, uint32_t reg_addr, size_t reg_size)
{
	struct tz_smmu_ipa_protect_region_iovec_s *unlock_vec;
	struct tz_smmu_ipa_protect_region_s cmd_buf;
	struct scm_desc desc = {0};
        const int size = sizeof(struct tz_smmu_ipa_protect_region_iovec_s);
	int ret;

	unlock_vec = kzalloc(PAGE_ALIGN(size), GFP_KERNEL);
	if (unlock_vec == NULL)
		return -ENOMEM;

        unlock_vec->input_addr = reg_addr ^ (reg_addr & 0xFFF);
        unlock_vec->output_addr = reg_addr ^ (reg_addr & 0xFFF);
        unlock_vec->size = reg_size;
        unlock_vec->attr = IPA_TZ_UNLOCK_ATTRIBUTE;

	cmd_buf.iovec_buf = virt_to_phys((void *) unlock_vec);
	cmd_buf.size_bytes = size;

	__cpuc_flush_dcache_area((void *) unlock_vec, size);
	outer_flush_range(cmd_buf.iovec_buf, cmd_buf.iovec_buf + size);

	if (!is_scm_armv8()) {
		if (__qcom_scm_is_call_available(dev, SCM_SVC_MP,
						TZ_MEM_PROTECT_REGION_ID)) {
                        int resp;
			ret = qcom_scm_call(dev, SCM_SVC_MP,
					TZ_MEM_PROTECT_REGION_ID,
            ...
            desc.args[0] = virt_to_phys(unlock_vec);
            desc.args[1] = size;
