...
/* For Msys BobK Sample At Reset the CPU, DDR and Main PLL clocks are all
...
static const u32 __initconst msys_bobk_cpu_frequencies[] = {
	400000000,
	1000000000,
	667000000,
	800000000,
...
static u32 __init msys_get_cpu_freq(void __iomem *sar)
{
	...
	if (of_machine_is_compatible("marvell,msys-bc2"))
		...
		cpu_freq = msys_bobk_cpu_frequencies[cpu_freq_select];
...
static const int __initconst msys_bobk_cpu_mpll_ratios[8][2] = {
	{1, 1}, {2, 1}, {3, 1}, {1, 1},
	{2, 1}, {1, 1}, {0, 1}, {0, 1},
};

static const int __initconst msys_bobk_cpu_ddr_ratios[8][2] = {
	{1, 1}, {2, 3}, {1, 1}, {1, 1},
	{2, 3}, {1, 2}, {0, 1}, {0, 1},
};
	...
	void __iomem *sar, int id, int *mult, int *div)
{
	...
	case MV98DX3236_CPU_TO_DDR:
		if (of_machine_is_compatible("marvell,msys-bc2")) {
			...
			*mult = msys_bobk_cpu_ddr_ratios[opt][0];
			*div = msys_bobk_cpu_ddr_ratios[opt][1];
		} else if (of_machine_is_compatible("marvell,msys-ac3")) {
			*mult = mv98dx3236_cpu_ddr_ratios[opt][0];
			*div = mv98dx3236_cpu_ddr_ratios[opt][1];
		}
	...
	case MV98DX3236_CPU_TO_MPLL:
		if (of_machine_is_compatible("marvell,msys-bc2")) {
			...
			*mult = msys_bobk_cpu_mpll_ratios[opt][0];
			*div = msys_bobk_cpu_mpll_ratios[opt][1];
		} else if (of_machine_is_compatible("marvell,msys-ac3")) {
			*mult = mv98dx3236_cpu_mpll_ratios[opt][0];
			*div = mv98dx3236_cpu_mpll_ratios[opt][1];
		}
