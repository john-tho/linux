	...
	unsigned int id)
{
	...
	case AL_FLASH_OBJ_ID_UEFI:		return "UEFI";
	...
	const char *str)
{
	...
	else if (!al_strcmp(str, "UEFI"))
		return AL_FLASH_OBJ_ID_UEFI;
	...
	al_bool is_extended)
{
	...
	if (is_extended) {
		al_print(
			"#  | OBJ ID                    | Instance | Name     | Device  | Offset   | Max Size | Valid | Version | Flavour\n");
		al_print(
			"------------------------------------------------------------------------------------------------------\n");
	} else {
		al_print(
	...
	for (i = 0; i < num_entries; i++) {
		...
		if (is_extended) {
			...
			al_print(" | %5c | %02u.%02u.%02u | %3lu",
				(err) ? 'X' : 'V', obj_hdr.major_ver,
				obj_hdr.minor_ver, obj_hdr.fix_ver,
				AL_FLASH_OBJ_ID_FLAVOUR(entry.obj_id));
		}
		al_print("\n");
	}
