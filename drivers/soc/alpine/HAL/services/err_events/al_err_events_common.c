 ...
 ** Field functions
 ******************************************************************************/
void al_err_events_common_field_set_cause(struct al_err_events_field *field)
{
	unsigned int ic_id;
	...
	if (!field->parent_module->ic_size) {
		al_warn("%s: Cannot set cause for module %s with ic_size == 0\n",
	...
	if (!field->parent_module->ic_regs_bases[ic_id])
		al_assert_msg(0, "%s: ic regs base is NULL for ic id %d\n", __func__, ic_id);

	al_iofic_set_cause(field->parent_module->ic_regs_bases[ic_id],
		field->iofic_group, field->iofic_bit);
}

void al_err_events_common_field_get_attrs(const struct al_err_events_field *field,
	al_bool *mask_status, al_bool *cause_status, al_bool *abort_status)
{
	unsigned int ic_id;
	unsigned int grp;
	...
	al_assert(field);
	al_assert(mask_status);
	al_assert(cause_status);
	al_assert(abort_status);

	if (!field->parent_module->ic_size) {
		al_warn("%s: Cannot read attributes for module %s with ic_size == 0\n",
			__func__, field->parent_module->name);
		return;
	}

	if (!field->valid) {
		al_warn("%s: Field %s is not valid\n", __func__, field->name);
		return;
	}
	...
	cause = al_iofic_read_cause(field->parent_module->ic_regs_bases[ic_id], grp);
	mask = al_iofic_read_mask(field->parent_module->ic_regs_bases[ic_id], grp);
	abort_mask = al_iofic_abort_mask_read(field->parent_module->ic_regs_bases[ic_id], grp);
...
unsigned int al_err_events_common_field_err_count_adj(struct al_err_events_field *field,
						      unsigned int count)
{
	unsigned int diff;
	...
	if (!field->ignore_count)
		return count;

	if (field->ignore_count >= count) {
		al_dbg("err_events: ERR %s,%s ignored %u times\n",
		       field->parent_module->name, field->name, count);
		field->ignore_count -= count;
		return 0;
	}

	al_dbg("err_events: ERR %s,%s ignored %u times\n",
	       field->parent_module->name, field->name, field->ignore_count);
	diff = count - field->ignore_count;
	field->ignore_count = 0;
	return diff;
}
...
void al_err_events_common_field_iofic_collect(struct al_err_events_field *field, uint32_t cause)
{
	...
	if (cause & field->iofic_bit) {
		...
		err_count = al_err_events_common_field_err_count_adj(field, 1);
		al_err_events_common_field_error(field, err_count);
