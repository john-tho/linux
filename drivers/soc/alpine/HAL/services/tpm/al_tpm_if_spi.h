...
/* Generic tpm spi interface struct to support TPM HAL service running under
 * existing spi HAL that provides spi functions (e.g. claim_bus, spi_xfer, etc)
 */
struct al_tpm_if_spi_gen {
...
/**
 * TPM SPI interface handle initialization using generic initialization
 * structure.
 *
 * @param	tpm_if
 *		An uninitialized TPM interface handle
 * @param	tpm_if_spi_gen
 *		TPM generic SPI interface
 * @param	init_params_gen
 *		Basic parameters for initialization
 */
