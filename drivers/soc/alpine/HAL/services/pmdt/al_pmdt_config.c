...
#define AL_PMDT_AXI_MON_FEATURES_MASK \
		(AL_PMDT_FEATURE_AXI_TIMEOUT | \
		AL_PMDT_FEATURE_AXI_LATENCY_HISTOGRAM | \
		AL_PMDT_FEATURE_AXI_TRAP | \
		AL_PMDT_FEATURE_AXI_TRACK)
	...
	uint32_t *event_mask)
{
	...
	event_config.trig_type = AL_PMDT_PMU_TRIG_TYPE_INT; /* internal trigger */
...
static int al_pmdt_pmu_ext_trig_events_config(
	struct al_pmdt_complex_handle *cmplx,
	int *event_num,
	uint32_t *event_mask,
	uint32_t *irq_mask)
{
	...
	/* -1 defines that no event was configured for external trigger 'i' */
	for (i = 0; i < AL_PMDT_EXT_TRIG_NUM; i++)
		cmplx->events_map.ext_trig_event_num[i] = -1;
	...
	/* configure trig event */
	for (i = 0; i < ((int)cmplx->pmu.events_num - *event_num) && i < AL_PMDT_EXT_TRIG_NUM; i++) {
		if (config->ext_trig_config.trig_list[i] == -1)
			break;

		trig_num = (int)config->ext_trig_config.trig_list[i];
		al_assert((trig_num >= 0) && (trig_num < AL_PMDT_EXT_TRIG_NUM));

		event_config.trig_num = (unsigned int)trig_num;
		cmplx->events_map.ext_trig_event_num[trig_num] = next_event_num;
	...
	/* check if we ran out of pmu event counters */
	if ((i >= (int)cmplx->pmu.events_num - *event_num) && (i < AL_PMDT_EXT_TRIG_NUM))
		return -ENOMEM;
	...
	struct al_pmdt_pmu_cmplx_info *cmplx_info)
{
	...
	if ((features & AL_PMDT_FEATURE_AXI_LATENCY_HISTOGRAM) ||
		(features & AL_PMDT_FEATURE_EXT_TRIG_TRACK)) {
	...
	if (features & AL_PMDT_FEATURE_EXT_TRIG_TRACK) {
		rc = al_pmdt_pmu_ext_trig_events_config(
				cmplx, &event_num, &event_mask, &groupa_irq_mask);
		if (rc == ENOMEM) {
			al_err("Out of pmu event resources for requested triggers\n");
			return rc;
		}
		al_assert(!rc);
	}
	...
	al_bool enable_interrupt)
{
	...
	uint32_t features;
	uint32_t cross_matrix_mask[AL_PMDT_CCTM_MATRIX_LINE_SIZE_MAX] = {0};
	...
	if (enable_interrupt) {
		...
		for (i = 0; i < pmdt->cctm.line_size; i++) {
	...
	int	*skip_unit)
{
	...
	if ((!cmplx->pmu.axi_mon_num) &&
			(cmplx->unit_config.feature_mask & AL_PMDT_AXI_MON_FEATURES_MASK)) {
		al_dbg("AXI_MON cannot supported by Complex\n");
		if (skip_unit)
			*skip_unit = 1;
	...
	cpu_clust_num = al_pmdt_map_cpu_clust_num_get(cmplx);
		...
		enum al_pmdt_unit unit)
{
	...
	if (setting->force_features)
		config->feature_mask = setting->force_features;

	if (setting->ext_trig) {
		if ((*setting->ext_trig)[0].valid) {
			for (i = 0; i < AL_PMDT_MAX_SETTING_EXT_TRIG; i++) {
				if ((*setting->ext_trig)[i].valid &&
					(*setting->ext_trig)[i].unit == unit &&
					(*setting->ext_trig)[i].trig < AL_PMDT_MAX_EXT_TRIG)
					config->ext_trig_config.trig_list[j++] =
							(*setting->ext_trig)[i].trig;
			}
			if (j < AL_PMDT_MAX_SETTING_EXT_TRIG)
				config->ext_trig_config.trig_list[j] = -1;
		}
	}
	...
	struct al_pmdt_config *config)
{
	...
	rc = al_pmdt_unit_complex_config_aux(pmdt, cmplx, unit, config, NULL);
...
{
	struct al_pmdt_complex_handle *cmplx;
	uint32_t pmu_mask[AL_PMDT_CCTM_MATRIX_LINE_SIZE_MAX] = {0};
	enum al_pmdt_unit unit;
	void __iomem *bs_regs = (void __iomem *)AL_PBS_REGFILE_BASE;
	struct al_pmdt_map_entry entry;
	struct al_pmdt_config config;
			...
			if (default_unit_enable && al_pmdt_is_unit_active(pmdt, unit)) {
				if (setting) {
					...
					al_memcpy(&config,
							&entry.pmdtu_config,
							sizeof(struct al_pmdt_config));
					rc = al_pmdt_config_settings(&config, setting, unit);
					al_assert(!rc);

					rc = al_pmdt_unit_complex_config_aux(
							pmdt, cmplx, unit, &config, &skip_unit);
				} else
					rc = al_pmdt_unit_complex_config_aux(
							pmdt, cmplx, unit, NULL, &skip_unit);
				al_assert(!rc);
