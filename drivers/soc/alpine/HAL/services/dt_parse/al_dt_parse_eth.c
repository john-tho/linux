	...
	al_bool *retimer_exists)
{
	...
	const uint32_t *val;
	uint32_t arr_len, i;
	uint8_t current_hw_id;
	int ret;

	struct al_uc_shared_handle uc_shared_handle;
	struct al_uc_shared_handle_init_params uc_shared_handle_init_params;
	struct al_uc_shared_fru_data fru_data = {0};

	al_memset(&uc_shared_handle, 0, sizeof(struct al_uc_shared_handle));
	al_memset(&uc_shared_handle_init_params, 0,
		sizeof(struct al_uc_shared_handle_init_params));
	...
	*retimer_exists = AL_FALSE;
	...
	ret = al_dt_parse_property_u32_get_array(handle, node_context, "hw-id-support",
		&val, &arr_len);
	/* If hw-id-support field does not exist - ignore the check */
	if (ret == 0) {

		uc_shared_handle_init_params.regs_base =
			(void __iomem *)(uintptr_t)AL_UC_SHARED_BASE;
		al_uc_shared_handle_init(&uc_shared_handle, &uc_shared_handle_init_params);
		al_uc_shared_fru_data_get(&uc_shared_handle, &fru_data);
		/* Reading fru 0xAC offset, which is [EC] field in VPD */
		current_hw_id = fru_data.pcb_rev_id[AL_UC_SHARED_PCB_REV_ID_LEN_BYTES - 1];

		for (i = 0; i < arr_len; i++)
			if (swap32_from_be(val[i]) == current_hw_id) {
				dt_debug(SERDES_LANE_RETIMER_PRI("supportts hw id %d\n", current_hw_id));
				break;
			}
		if (i == arr_len) {
			dt_debug(SERDES_LANE_RETIMER_PRI("does not support hw id %d\n", current_hw_id));
			return;
		}
	}
