...
/**
 * Get a node property array of  unsigned 32-bit values from a given node context
 *
 * @param handle
 *      DT Parse handle
 * @param node_context
 ...
 *      Name of the property inside the node
 * @param val
 *      Pointer to an 32-bit pointer, to be filled with actual address inside DT
 * @param arr_len
 *      Pointer to an unsigned 32-bit value, to be filled with number of
 *      unsigned 32-bit values in the property
 *
 * @return
 *      0 on success, errno otherwise.
 */
int al_dt_parse_property_u32_get_array(
	const struct al_dt_parse_handle *handle,
	const void *node_context,
	const char *property,
	const uint32_t **val,
