...
#define AL_UC_GET_SECURE_MODE_KR	0x99999999
	...
	struct al_uc_handle_init	*init)
{
	...
	if (init->timeout_ms == 0)
		handle->timeout_ms = AL_UC_HOSTCMD_TIMEOUT_MSEC;
	else
		handle->timeout_ms = init->timeout_ms;

	al_uc_gpio_handle_init(handle, &uc_gpio);
}

/*
 * @param verbose - AL_TRUE: full printouts, AL_FALSE: no printouts
 */
static int al_uc_cmd_adv(
	...
	al_bool			verbose)
{
	struct uc_cmd_regs *cmd_regs;
	struct al_gpio_interface gpio_if;
	int pin_idx;
	int err = 0;

	if (handle == NULL || resp_param == NULL)
		return -EINVAL;

	cmd_regs = (struct uc_cmd_regs *)handle->cmd_regs_base;
	...
	if (!timeout_ms) {
		...
		if (verbose == AL_TRUE) {
			...
			if (cmd_regs->response == AL_UC_RESPONSE_SECURE_NOT_SUPPORTED)
				al_err("Command is not supported in secure-mode.\n");
		}
		return -EIO;
	}
	...
	uint32_t		*resp_param)
{
	return al_uc_cmd_adv(handle, opcode, param, AL_UC_RESPONSE_DONE, resp_param,
		handle->timeout_ms, AL_TRUE);
}

int al_uc_cmd_timeout(
	struct al_uc_handle	*handle,
	uint32_t		opcode,
	uint32_t		param,
	uint32_t		*resp_param,
...
int al_uc_app_version_get_ex(
	struct al_uc_handle	*handle,
	uint32_t		*ver,
	struct al_uc_cmds_version_params	*params)
{
	al_assert(params);

	int err = al_uc_cmd_adv(handle, AL_UC_OPCODE_APP_VERSION, 0, AL_UC_RESPONSE_DONE,
		ver, handle->timeout_ms, params->verbose);

	return err;
}

int al_uc_bl_version_get_ex(
	struct al_uc_handle	*handle,
	uint32_t		*ver,
	struct al_uc_cmds_version_params	*params)
{
	...
	int err = al_uc_cmd_adv(handle, AL_UC_OPCODE_BL_VERSION, 0, AL_UC_RESPONSE_DONE, ver,
		 handle->timeout_ms, params->verbose);
...
int al_uc_app_version_get(
	...
	uint32_t		*ver)
{
	struct al_uc_cmds_version_params params = {0};

	params.verbose = AL_TRUE;
	return al_uc_app_version_get_ex(handle, ver, &params);
}
	...
	uint32_t		*ver)
{
	...
	params.verbose = AL_TRUE;
	return al_uc_bl_version_get_ex(handle, ver, &params);
}
	...
	unsigned int		timeout_ms)
{
	...
	err = al_uc_cmd_adv(handle, opcode, image_phys_addr,
				AL_UC_RESPONSE_SW_UPDATE_OK, &resp_param, timeout_ms, AL_TRUE);
	return err;
}
	...
	unsigned int		image_size)
{
	...
	if (image_size == IMAGE_SIZE_12K)
	...
	else if (image_size == IMAGE_SIZE_41_5K || image_size == IMAGE_SIZE_41_5K + AL_UC_SIGNATURE_SIZE ||
			 image_size == IMAGE_SIZE_43K || image_size == IMAGE_SIZE_43K + AL_UC_SIGNATURE_SIZE ||
			 image_size == IMAGE_SIZE_44K)
		...
		al_err("image file size is supposed to be %lu / %lu / %lu / %lu\n",
			(unsigned long)IMAGE_SIZE_12K,
			(unsigned long)IMAGE_SIZE_192K,
			(unsigned long)IMAGE_SIZE_41_5K,
	...
	uint32_t		delay_before_reset)
{
	...
	if (handle == NULL || handle->cmd_regs_base == NULL)
		return -EINVAL;
	...
	err = al_uc_cmd_adv(handle, AL_UC_OPCODE_RESET, delay_before_reset, AL_UC_RESPONSE_DONE,
				&resp_param, handle->timeout_ms, AL_TRUE);
	...
	unsigned int		*i2c_addr)
{
	...
	if (en == NULL || i2c_addr == NULL)
		return -EINVAL;

	err = al_uc_cmd(handle, AL_UC_OPCODE_THERMAL_GET, 0, &resp_param);
	...
	unsigned int		*i2c_addr)
{
	...
	err = al_uc_cmd(handle, AL_UC_OPCODE_MGMT_GET, 0, &resp_param);
...
int al_uc_reset_mdelay_get(
	struct al_uc_handle	*handle,
	uint16_t		*reset_mdelay)
{
	uint32_t resp_param;
	int err;

	if (reset_mdelay == NULL)
		return -EINVAL;

	err = al_uc_cmd(handle, AL_UC_OPCODE_RESET_MDELAY_GET, 0, &resp_param);

	if (!err)
		*reset_mdelay = resp_param;

	return err;
}

int al_uc_reset_mdelay_set(
	struct al_uc_handle	*handle,
	uint16_t		reset_mdelay)
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_RESET_MDELAY_SET, reset_mdelay, &resp_param);

	return err;
}

int al_uc_dt_flavour_get(
	struct al_uc_handle	*handle,
	uint16_t		*dt_flavour)
{
	uint32_t resp_param;
	...
	err = al_uc_cmd(handle, AL_UC_OPCODE_GET_DT_FLAVOUR, 0, &resp_param);

	if (!err)
		*dt_flavour = resp_param;

	return err;
}

int al_uc_dt_flavour_set(
	struct al_uc_handle	*handle,
	uint16_t		dt_flavour)
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_SET_DT_FLAVOUR, dt_flavour, &resp_param);

	return err;
}

int al_uc_serdes15_get(
	struct al_uc_handle     *handle,
	uint8_t                *val)
{
	uint32_t resp_param;
	...
	err = al_uc_cmd(handle, AL_UC_OPCODE_GET_SERDES15_FORCE, 0, &resp_param);

	if (!err)
		*val = (uint8_t)resp_param;

	return err;
}

int al_uc_serdes15_set(
	struct al_uc_handle     *handle,
	uint8_t                val)
{
	uint32_t resp_param;

	return al_uc_cmd(handle, AL_UC_OPCODE_SET_SERDES15_FORCE, val, &resp_param);
}

int al_uc_apcea_cfg_flavour_get(
	struct al_uc_handle	*handle,
	uint16_t		*apcea_cfg_flavour)
{
	uint32_t resp_param;
	int err;

	if (apcea_cfg_flavour == NULL)
		return -EINVAL;

	err = al_uc_cmd(handle, AL_UC_OPCODE_GET_APCEA_CFG_FLAVOUR, 0, &resp_param);

	if (!err)
		*apcea_cfg_flavour = resp_param;

	return err;
}

int al_uc_apcea_cfg_flavour_set(
	struct al_uc_handle	*handle,
	uint16_t		apcea_cfg_flavour)
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_SET_APCEA_CFG_FLAVOUR, apcea_cfg_flavour, &resp_param);

	return err;
}

int al_uc_pxe_flavour_get(
	struct al_uc_handle	*handle,
	uint8_t			*pxe_flavour)
{
	uint32_t resp_param;
	...
	err = al_uc_cmd(handle, AL_UC_OPCODE_GET_PXE_FLAVOUR, 0, &resp_param);

	if (!err)
		*pxe_flavour = resp_param;

	return err;
}

int al_uc_pxe_flavour_set(
	struct al_uc_handle	*handle,
	uint8_t			pxe_flavour)
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_SET_PXE_FLAVOUR, pxe_flavour, &resp_param);
	...
	unsigned int		*val)
{
	...
	err = al_uc_cmd(handle, AL_UC_OPCODE_GPIO_READ, param, &resp_param);
	...
	al_bool			*is_output)
{
	...
	err = al_uc_cmd(handle, AL_UC_OPCODE_GPIO_GET_DIR, param, &resp_param);
	...
	unsigned int		*timeout)
{
	...
	if (en == NULL || rst_cnt == NULL || timeout == NULL)
		return -EINVAL;

	err = al_uc_cmd(handle, AL_UC_OPCODE_WATCHDOG_GET, 0, &resp_param);
	en_raw = ((resp_param & AL_UC_WATCHDOG_GET_DIS_MASK) >>
	...
	al_bool			is_enabled)
{
	...
	int err = al_uc_cmd(handle, AL_UC_OPCODE_SET_SECURE, param, &resp_param);
	...
	al_bool			is_enabled)
{
	uint32_t resp_param;
	uint32_t param = is_enabled;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_SET_PP, param, &resp_param);
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_SDR_RESET, 0, &resp_param);
	...
	int			buffer_size)
{
	...
	if (sdr_data == NULL || num_bytes == NULL)
		return -EINVAL;
...
int al_uc_get_secure_kr(
	struct al_uc_handle	*handle,
	uint32_t		*resp_param)
{
	uint32_t param = AL_UC_GET_SECURE_MODE_KR;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_GET_SECURE, param, resp_param);
...
{
	uint32_t param = 0;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_GET_PP, param, resp_param);
...
{
	uint32_t param = 0;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_GET_SECURE_LOG, param, resp_param);
...
int al_uc_clear_secure_log(struct al_uc_handle	*handle)
{
	uint32_t resp_param;
	uint32_t param = 0;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_CLEAR_SECURE_LOG, param, &resp_param);
	...
	uint32_t		*thermal_shutdown_status)
{
	...
	if (thermal_shutdown_status == NULL)
		return -EINVAL;
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, HOSTCMD_OPCODE_SET_SPI_BUFFER, is_enabled, &resp_param);
	...
	unsigned int            thermal_shutdown_max_temp)
{
	...
	int err = al_uc_cmd(handle, AL_UC_OPCODE_THERMAL_SHUTDOWN_SET_TMP, param, &resp_param);
	...
	unsigned int		*thermal_shutdown_max_temp)
{
	...
	if (thermal_shutdown_max_temp == NULL)
		return -EINVAL;
...
{
	int err = al_uc_cmd(handle, HOSTCMD_OPCODE_NITRO_MODE_GET, 0, nitro_mode_status);
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_FRU_ERASE, 0, &resp_param);
	...
	int			buffer_size)
{
	...
	if (fru_data == NULL || num_bytes == NULL)
		return -EINVAL;

	*num_bytes = 0;

	for (i = 0; i < buffer_size; i++) {
		err = al_uc_cmd_adv(handle, AL_UC_OPCODE_FRU_PRINT, i, AL_UC_RESPONSE_DONE,
				&resp_param, handle->timeout_ms, AL_FALSE);
		if (err) {
			if (err == -ETIME)
				al_err("%s(0x%lX): timeout!\n", __func__, AL_UC_OPCODE_FRU_PRINT);
			if (err != -EIO)
	...
	al_phys_addr_t		image_phys_addr)
{
	...
	*(uint32_t *)p = image_size;
	...
	err = al_uc_cmd_adv(handle, AL_UC_OPCODE_FRU_BURN, image_phys_addr,
				AL_UC_RESPONSE_DONE, &resp_param, handle->timeout_ms, AL_TRUE);
	return err;
}
	...
	unsigned int		*i2c_address)
{
	...
	if (en == NULL || i2c_address == NULL)
		return -EINVAL;

	err = al_uc_cmd(handle, AL_UC_OPCODE_SDR_GET, 0, &resp_param);
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_SDR_SET_FW_UPDATE, data, &resp_param);
	...
	unsigned int		*i2c_address)
{
	...
	err = al_uc_cmd(handle, AL_UC_OPCODE_FRU_STATUS, 0, &resp_param);
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_CFG_MEM_ERR_SET, data, &resp_param);
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_CFG_MEM_ERR_CLEAR, 0, &resp_param);
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_FRU_SRAM_READY, 0, &resp_param);
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_UC_UART_TEST, en, &resp_param);
	...
	uint32_t		*val)
{
	...
	err = al_uc_cmd(handle, AL_UC_OPCODE_SPI_FLASH_GET, 0, &resp_param);
	...
	uint32_t		*val)
{
	...
	err = al_uc_cmd(handle, AL_UC_OPCODE_SPI_BOOTED_FROM, 0, &resp_param);
	*val = resp_param;
	...
	uint32_t		*val)
{
	...
	err = al_uc_cmd(handle, AL_UC_OPCODE_CPU_VOLTAGE_GET, 0, &resp_param);
	*val = resp_param;
...
int al_uc_liveness_timer_get(
	...
	uint32_t		*timer)
{
	int err = al_uc_cmd(handle, AL_UC_OPCODE_GET_LIVENESS_TIMER, 0, timer);
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_FRU_UNLOCK, 0, &resp_param);
	...
	unsigned int size)
{
	...
	if (buf == NULL || (size > AL_UC_SPI_PRELOADER_MAX_SIZE))
		return -EINVAL;

	/*
	 * Read preloader data from uC Flash.
	 * uC read function will return a DWord per invocation, while the byte buffer
	 * is filled, one byte per iteration.
	 */

	for (i = 0; i < size; i++) {
		/* This is the bytewise index for the DWord read buffer */
		const int byte_indx_to_dword_indx = i % sizeof(uint32_t);

		if (!byte_indx_to_dword_indx) {
			err = al_uc_cmd(handle, AL_UC_OPCODE_SPI_PREALODER_READ, (i / sizeof(uint32_t)),
						&resp_param.resp_param_uint32);
			if (err)
				return err;
		}
		buf[i] = resp_param.resp_param_uint8[byte_indx_to_dword_indx];
	}
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_UC_UART_PHYS_LB, 0, &resp_param);
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_SDR_SET_REG_MASK, reg_num, &resp_param);
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_SDR_CLEAR_REG_MASK, reg_num, &resp_param);
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_ALPACA_CONFIG_TEST, value, &resp_param);

	return err;
}

int al_uc_app_idx_get_ex(
	struct al_uc_handle *handle,
	uint32_t *app_idx,
	struct al_uc_cmds_idx_params *params)
{
	uint32_t resp_param;
	int err;
	...
	err = al_uc_cmd_adv(handle, AL_UC_OPCODE_APP_IDX_GET, 0, AL_UC_RESPONSE_DONE, &resp_param,
		AL_UC_HOSTCMD_TIMEOUT_MSEC, params->verbose);
	*app_idx = resp_param;
...
int al_uc_app_idx_get(
	...
	uint32_t *app_idx)
{
	struct al_uc_cmds_idx_params params = {0};

	params.verbose = AL_TRUE;
	return al_uc_app_idx_get_ex(handle, app_idx, &params);
}
...
{
	uint32_t resp_param;
	int err = al_uc_cmd(handle, AL_UC_OPCODE_APP_IDX_SET, app_idx, &resp_param);

	return err;
}

int al_uc_ep_rc_auto_detect_set(
	struct al_uc_handle *handle,
	unsigned int val)
{
	uint32_t resp_param;
	...
	if ((val != 0) && (val != 1) && (val != 0xFF))
		return -EINVAL;

	err = al_uc_cmd(handle, AL_UC_OPCODE_EP_RC_AUTO_DETECT_SET, (uint32_t)val, &resp_param);

	return err;
}

int al_uc_ep_rc_auto_detect_get(
	struct al_uc_handle *handle,
	uint32_t *mode)
{
	int err = al_uc_cmd(handle, AL_UC_OPCODE_EP_RC_AUTO_DETECT_GET, (uint32_t)0, mode);
