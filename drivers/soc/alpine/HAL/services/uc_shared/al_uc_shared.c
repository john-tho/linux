...
void al_uc_shared_current_sensor_get(const struct al_uc_shared_handle *handle,
				     ...
				     uint16_t *electric_voltage)
{
	...
	al_assert(handle);
	al_assert(channel_idx < AL_UC_SHARED_NUM_OF_BUS_VOLT);
	al_assert(electric_current);
	al_assert(electric_voltage);

	shared_data = (struct al_uc_shared_data *)(handle->regs_base);

	*electric_current = shared_data->telemetry.shunt_volt[channel_idx];
	*electric_voltage = shared_data->telemetry.bus_volt[channel_idx];
}
