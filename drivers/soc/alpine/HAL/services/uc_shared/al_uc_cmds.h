...
#define AL_UC_OPCODE_STACK_CHECK		0xBABAB1B1UL
...
#define AL_UC_OPCODE_EP_RC_AUTO_DETECT_SET	0xBABA9C9CUL
#define AL_UC_OPCODE_EP_RC_AUTO_DETECT_GET	0xBABA9D9DUL

#define AL_UC_OPCODE_RESET_MDELAY_SET		0xBABA8C8CUL
#define AL_UC_OPCODE_RESET_MDELAY_GET		0xBABA8D8DUL

#define AL_UC_OPCODE_SET_DT_FLAVOUR			0xBABA8E8EUL
#define AL_UC_OPCODE_GET_DT_FLAVOUR			0xBABA8F8FUL

#define AL_UC_OPCODE_SET_APCEA_CFG_FLAVOUR	0xBABA9090UL
#define AL_UC_OPCODE_GET_APCEA_CFG_FLAVOUR	0xBABA9191UL

#define AL_UC_OPCODE_GET_PXE_FLAVOUR		0xBABA9494UL
#define AL_UC_OPCODE_SET_PXE_FLAVOUR		0xBABA9595UL

#define AL_UC_OPCODE_GET_SERDES15_FORCE		0xBABA9696UL
#define AL_UC_OPCODE_SET_SERDES15_FORCE		0xBABA9797UL
...
#define AL_UC_OPCODE_KR_EXIT_SECURE		0xBABA4646UL
#define AL_UC_OPCODE_KR_IS_EXIT_SECURE		0xBABA4747UL
#define AL_UC_OPCODE_KR_TPM_CLEARED		0xBABA4848UL

#define AL_UC_RESPONSE_DONE			0xCACA00AAUL
#define AL_UC_RESPONSE_FAIL			0xCACAFAFAUL
#define AL_UC_RESPONSE_SECURE_NOT_SUPPORTED	0xCACAFCFCUL
...
#define IMAGE_SIZE_41_5K			((41 * 1024) + 512)
...
#define AL_UC_KR_TPM_CLEAR_HOSTCMD_TIMEOUT_MSEC		(15 * 1000)
#define AL_UC_KR_STG2_SIGN_HOSTCMD_TIMEOUT_MSEC		(40 * 1000)
#define AL_UC_KR_SBK_IMPORT_HOSTCMD_TIMEOUT_MSEC	(40 * 1000)
...
#define AL_UC_SBK_IMPORT_SIZE		1280
...
struct al_uc_handle_init {
	...
	unsigned int	timeout_ms; /**< uc command gpio timeout in ms */
};
...
/** al_uc_cmds_version_params */
struct al_uc_cmds_version_params {
	al_bool verbose; /**< AL_TRUE: full printouts, AL_FALSE: no printouts */
};

/** al_uc_cmds_idx_params */
struct al_uc_cmds_idx_params {
	al_bool verbose; /**< AL_TRUE: full printouts, AL_FALSE: no printouts */
};
...
/**
 * send host command to uC with custom timeout
 * @param handle uC handle
 * @param opcode host command opcode
 * @param param	param to pass to uC
 * @param resp_param response param from uC
 * @param timeout_ms custom timeout in ms
 *
 * @return 0 upen success, other value upon failure
 */
 ...
 * @param  handle - uc controller handle
 * @param  resp_param - The secure state according to kr state machine
 * return  0 on success. Other value on fail.
 */
...
/**
 * Get the secure state - for uc supporting key rotation feature
 * @param  handle - uc controller handle
 * @param  resp_param - The secure state according to kr state machine
 * return  0 on success. Other value on fail.
 */
...
/**
 * Set EP/RC auto-detect mode
 * @param handle - uc controller handle
 * @param val - 0 - boot from main flash if PCIE_WAKE signal is 0 and from recovery flash otherwise
 *		1 - boot from recovery flash if PCIE_WAKE signal is 0 and from main flash otherwise
 *		0xFF - Disable auto detect
 * @return 0 on success
 */
...
/**
 * Get EP/RC auto-detect mode
 * @param handle - uc controller handle
 * @param mode - The returned mode
 *		0 - boot from main flash if PCIE_WAKE signal is 0 and from recovery flash otherwise
 *		1 - boot from recovery flash if PCIE_WAKE signal is 0 and from main flash otherwise
 *		0xFF - Disable auto detect
 * @return 0 on success
 */
