...
struct udma_gen_section_ctrl_transaction_type_table {
	...
	/* [0x1c] Enable flush mode and store mode */
	uint32_t wqe_mode;
};
...
/**** wqe_mode register ****/
/*
 * Qualified only at EOP
 * When set, UDMA will consume (pop) current desc if any space left,
 * and will continue consume descriptor until arriving with a desc with a flush marker set. (bit
 * inside of the desc)
 * Reset: 0x0         Access: RW
 */
#define UDMA_GEN_EX_TRANSACTION_TYPE_TABLE_WQE_MODE_FLUSH_EN (1 << 0)
#define UDMA_GEN_EX_TRANSACTION_TYPE_TABLE_WQE_MODE_FLUSH_EN_MASK 0x00000001
#define UDMA_GEN_EX_TRANSACTION_TYPE_TABLE_WQE_MODE_FLUSH_EN_SHIFT 0
/*
 * Qualified only at EOP
 * When set, UDMA will not "pop" working desc and next packet arrive to same queue will continue
 * from current  offset in current desc.
 * For example first packet size was 0x100, and desc addr is 0xa00,
 * when append is set, next packet on same queue will continue to write,
 * from address 0xb00 (0x100 + 0xa00)
 * Reset: 0x0         Access: RW
 */
#define UDMA_GEN_EX_TRANSACTION_TYPE_TABLE_WQE_MODE_APPEND_EN (1 << 1)
#define UDMA_GEN_EX_TRANSACTION_TYPE_TABLE_WQE_MODE_APPEND_EN_MASK 0x00000002
#define UDMA_GEN_EX_TRANSACTION_TYPE_TABLE_WQE_MODE_APPEND_EN_SHIFT 1
/*
 * Qualified only at EOP  When set, and q work in block mode, this will mark next cmpl with last bit
 * to close block
 * Reset: 0x0         Access: RW
 */
#define UDMA_GEN_EX_TRANSACTION_TYPE_TABLE_WQE_MODE_END_OF_BLOCK_SET (1 << 2)
#define UDMA_GEN_EX_TRANSACTION_TYPE_TABLE_WQE_MODE_END_OF_BLOCK_SET_MASK 0x00000004
#define UDMA_GEN_EX_TRANSACTION_TYPE_TABLE_WQE_MODE_END_OF_BLOCK_SET_SHIFT 2
