...
static struct device_attribute __attribute__((unused)) subsys_attrs[] = {
	__ATTR_RO(name),
	__ATTR_RO(state),
...
static void subsys_device_release(struct device *dev)
{
	...
	del_timer_sync(&subsys->ssr_wlock.timer);
...
struct subsys_device *subsys_register(struct subsys_desc *desc)
{
	...
	memset(&subsys->ssr_wlock, 0, sizeof(subsys->ssr_wlock));
	subsys->ssr_wlock.name = subsys->wlname;
