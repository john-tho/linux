...
static int glink_smem_native_probe(struct platform_device *pdev)
{
	...
	uint32_t irq_line;
	uint32_t irq_mask;
	size_t ch_desc_size = SMEM_CH_DESC_SIZE;
	uint32_t entry = 0;
	uint32_t remote_proc_id;
	struct resource *r;
	...
	einfo->out_irq_reg = ioremap(r->start, resource_size(r));
...
static int glink_rpm_native_probe(struct platform_device *pdev)
{
	...
	einfo->out_irq_mask = irq_mask;
	einfo->out_irq_reg = ioremap(irq_r->start, resource_size(irq_r));
	if (!einfo->out_irq_reg) {
		pr_err("%s: unable to map irq reg\n", __func__);
		rc = -ENOMEM;
	...
	msgram = ioremap(msgram_r->start, resource_size(msgram_r));
	if (!msgram) {
...
static int glink_mailbox_probe(struct platform_device *pdev)
{
	...
	mbox_loc = ioremap(mbox_loc_r->start, resource_size(mbox_loc_r));
	if (!mbox_loc) {
	...
	mbox_size = ioremap(mbox_size_r->start, resource_size(mbox_size_r));
	if (!mbox_size) {
	...
	einfo->rx_reset_reg = ioremap(rx_reset_r->start, resource_size(rx_reset_r));
	if (!einfo->rx_reset_reg) {
