...
static unsigned long cmd_offset = 0;

#define MSM_NAND_DEV_CMD1(info)		(MSM_NAND_DEV_CMD1_UNADJ(info) + cmd_offset) 
#define MSM_NAND_DEV_CMD_VLD(info)	(MSM_NAND_DEV_CMD_VLD_UNADJ(info) + cmd_offset)

extern struct clk *of_clk_get_by_name(struct device_node *np, const char *name);
				...
				bool vote)
{
	...
	if (IS_ERR_OR_NULL(info->clk_data.qpic_clk) ||
			IS_ERR_OR_NULL(info->clk_data.qpic_ahb_clk)) {
		ret = -EINVAL;
		goto out;
	}
	...
	if (!atomic_read(&info->clk_data.clk_enabled) && vote) {
		...
		ret = clk_prepare_enable(info->clk_data.qpic_ahb_clk);
		if (ret) {
			pr_err("Failed to enable the iface-clock with error %d\n",
				ret);
			msm_nand_bus_set_vote(info, 0);
			goto out;
		}
		...
		clk_disable_unprepare(info->clk_data.qpic_ahb_clk);
		msm_nand_bus_set_vote(info, 0);
	}
...
static int msm_nand_flash_onfi_probe(struct msm_nand_info *info)
{
	...
	if (of_machine_is_compatible("qcom,ipq4019") &&
	    (!ret && !(nandc_version.nand_major == 1 &&
...
static int msm_nand_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	if ((uint32_t)instr->len != mtd->erasesize) {
		pr_err("unsupported erase len, %ud\n", (uint32_t)instr->len);
		err = -EINVAL;
		goto out;
	}
...
static int msm_nand_block_isbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	struct sps_iovec *iovec;
	struct sps_iovec iovec_temp;
	uint32_t total_cnt = 10;

	/*
	 * The following 10 commands are required to check bad block -
...
static int nand_read_fact(struct mtd_info *mtd, loff_t from,
	...
	if (from == 0) {
		/* return nand id */
		*retlen = min((unsigned)len, sizeof(info->flash_dev.flash_id));
		memset(buf, 0, len);
		memcpy(buf, &info->flash_dev.flash_id, *retlen);
		return 0;
	}
	return -ESRCH;
}
...
//		}
		...
		if (flash_id == 0x2690ac98 || flash_id == 0x2680ac2c){
...
static int msm_nand_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
	int err;
	int skipClockSetup = 0;
	unsigned adjustment_offset = 0;
#ifdef CONFIG_MSM_SMD
	int i, nr_parts;
#endif
...
#endif
	if (of_machine_is_compatible("qcom,sdx20")) {
		msleep(200); // let fixup RPM clocks
	}
	...
	info->clk_data.qpic_ahb_clk = devm_clk_get(&pdev->dev, "iface_clk");

	skipClockSetup = of_property_read_bool(pdev->dev.of_node, "skip-clocks");
	...
	if (!IS_ERR_OR_NULL(info->clk_data.qpic_clk)) {
		...
		if (skipClockSetup) {
			pr_info("(!)continue without clockctl\n");
			err = 0;
		}
	}
	if (err)
	...
	if (of_machine_is_compatible("qcom,ipq807x")) {
		info->clk_data.iface_clk = of_clk_get_by_name(pdev->dev.of_node, "iface_clk");
		if (!IS_ERR_OR_NULL(info->clk_data.iface_clk))
			clk_prepare_enable(info->clk_data.iface_clk);
	}
	if (!skipClockSetup) {
		err = msm_nand_setup_clocks_and_bus_bw(info, true);
		if (err) {
			printk("msm_nand: clk setup failed\n");
			goto bus_unregister;
		}
	}
...
//	printk("msm_nand: qpic_version=%d\n", qpic_version.qpic_major);
	...
	pr_info("Allocated DMA buffer at virt_addr 0x%px, phys_addr 0x%x\n",
		info->nand_chip.dma_virt_addr, info->nand_chip.dma_phys_addr);
#ifdef CONFIG_MTD_NAND_RB
	mtd_set_of_node(&info->mtd, pdev->dev.of_node);
