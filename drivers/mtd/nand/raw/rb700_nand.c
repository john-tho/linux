...
static void nolatch_select_chip(struct nand_chip *chip, int chipnr);
...
static int rb700_dev_ready(struct nand_chip *chip) {
...
static void rb700_select_chip(struct nand_chip *chip, int chipnr)
{
	int spi_retcode;
	unsigned fn = GPIO_REG(GPIO_FN_REG);
	if (chipnr >= 0) {
		if (spi) {
			spi_retcode =  spi_sync(spi, &m_acquire);
			if (spi_retcode) {
				printk("warn: spi-cs failed %d\n",
						spi_retcode);
			}
                }
		...
		if (spi) {
			spi_retcode = spi_sync(spi, &m_release);
			if (spi_retcode) {
				printk("warn: spi-cs-release failed %d\n",
					spi_retcode);
			}
                }
	}
...
static void rb700_hwcontrol(struct nand_chip *chip, int cmd,
			     unsigned int ctrl) {
...
static uint8_t rb700_read_byte(struct nand_chip *chip)
{
	uint8_t data;
...
static void rb700_write_buf(struct nand_chip *chip, const uint8_t *buf, int len)
{
	rb700_write_bytes_gpio(buf, len);
}

static void rb700_read_buf(struct nand_chip *chip, uint8_t *buf, int len)
{
...
static int rb700_nand_probe(struct platform_device *pdev)
{
	...
	rnand.legacy.select_chip = no_latch ? nolatch_select_chip
					    : rb700_select_chip;
	rnand.legacy.cmd_ctrl = rb700_hwcontrol;
	rnand.legacy.dev_ready = rb700_dev_ready;
	rnand.legacy.read_byte = rb700_read_byte;
	rnand.legacy.write_buf = rb700_write_buf;
	rnand.legacy.read_buf = rb700_read_buf;
...
static int rb700_spi_probe(struct spi_device *_spi) {
	...
	static struct spi_transfer st_rel = {
		.len = 0,
		.cs_change = 0,
	};
	...
	spi_message_add_tail(&st_rel, &m_release);
	return 0;
}
