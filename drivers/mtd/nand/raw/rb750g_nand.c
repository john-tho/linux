...
static int rb750g_dev_ready(struct nand_chip *chip) {
...
static void rb750g_hwcontrol(struct nand_chip *chip, int cmd,
			     unsigned int ctrl) {
...
static uint8_t rb750g_read_byte(struct nand_chip *chip)
{
	uint8_t data;
...
static void rb750g_write_buf(struct nand_chip *chip,
			     const uint8_t *buf, int len)
{
	rb750g_write_bytes_gpio(buf, len);
}

static void rb750g_read_buf(struct nand_chip *chip, uint8_t *buf, int len)
{
...
static int rb750g_nand_probe(struct platform_device *pdev)
{
	...
	rnand.legacy.cmd_ctrl = rb750g_hwcontrol;
	rnand.legacy.dev_ready = rb750g_dev_ready;
	rnand.legacy.read_byte = rb750g_read_byte;
	rnand.legacy.write_buf = rb750g_write_buf;
	rnand.legacy.read_buf = rb750g_read_buf;
