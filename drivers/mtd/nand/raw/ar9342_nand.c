...
static void ar9342_ssr_cfg_callback(void *ptr) {
	...
	if (tx.tx_buf == NULL) {
		...
		m_upd.cs_release_after_complete = 1;
...
static int ar9342_dev_ready(struct nand_chip *chip) {
...
static void ar9342_hwcontrol(struct nand_chip *chip, int cmd,
			     unsigned int ctrl) {
...
static uint8_t ar9342_read_byte(struct nand_chip *chip)
{
	uint8_t data;
...
static void ar9342_write_buf(struct nand_chip *chip,
			     const uint8_t *buf, int len)
{
	ar9342_write_bytes_gpio(buf, len);
}

static void ar9342_read_buf(struct nand_chip *chip, uint8_t *buf, int len)
{
...
static int ar9342_nand_probe(struct platform_device *pdev)
{
	...
	rnand.legacy.cmd_ctrl = ar9342_hwcontrol;
	rnand.legacy.dev_ready = ar9342_dev_ready;
	rnand.legacy.read_byte = ar9342_read_byte;
	rnand.legacy.write_buf = ar9342_write_buf;
	rnand.legacy.read_buf = ar9342_read_buf;
