...
static int ar9330_dev_ready(struct nand_chip *chip) {
...
static void ar9330_hwcontrol(struct nand_chip *chip, int cmd,
			     unsigned int ctrl) {
...
static uint8_t ar9330_read_byte(struct nand_chip *chip)
{
	uint8_t data;
...
static void ar9330_write_buf(struct nand_chip *chip,
			     const uint8_t *buf, int len)
{
	ar9330_write_bytes_gpio(buf, len);
}

static void ar9330_read_buf(struct nand_chip *chip, uint8_t *buf, int len)
{
...
static int ar9330_nand_probe(struct platform_device *pdev)
{
	...
	rnand.legacy.cmd_ctrl = ar9330_hwcontrol;
	rnand.legacy.dev_ready = ar9330_dev_ready;
	rnand.legacy.read_byte = ar9330_read_byte;
	rnand.legacy.write_buf = ar9330_write_buf;
	rnand.legacy.read_buf = ar9330_read_buf;
