...
static int rbppc_nand_ready(struct nand_chip *chip) {
	struct rbppc_info *priv = nand_get_controller_data(chip);

	return in_be32(priv->gpi) & priv->gpio_rdy;
}

static void rbppc_nand_hwcontrol(struct nand_chip *chip,
	...
	if (cmd != NAND_CMD_NONE) writeb(cmd, chip->legacy.IO_ADDR_W);
}

static void rbppc_read_buf(struct nand_chip *chip, uint8_t *buf, int len)
{
	memcpy(buf, chip->legacy.IO_ADDR_R, len);
}
...
static int rbppc_nand_probe(struct platform_device *pdev)
{
	...
	info->gpo = ioremap(res.start, res.end - res.start + 1);

	if (!of_address_to_resource(gpio, 1, &res)) {
		info->gpi = ioremap(res.start, res.end - res.start + 1);
	} else {
		info->gpi = info->gpo;
	}
	of_node_put(gpio);
	...
	of_node_put(nnand);
	info->localbus = ioremap(res.start, res.end - res.start + 1);
	
	if (of_address_to_resource(pdev->dev.of_node, 0, &res)) {
	    printk("rbppc nand error: no reg property found\n");
	    goto err;
	}
	baddr = ioremap(res.start, res.end - res.start + 1);

	memset(&rnand, 0, sizeof(rnand));
	rnand.legacy.cmd_ctrl = rbppc_nand_hwcontrol;
    	rnand.legacy.dev_ready = rbppc_nand_ready;
    	rnand.legacy.read_buf = rbppc_read_buf;
	rnand.legacy.IO_ADDR_W = baddr;
	rnand.legacy.IO_ADDR_R = baddr;
	rnand.priv = info;
