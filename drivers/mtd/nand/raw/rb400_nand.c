...
static int rb400_dev_ready(struct nand_chip *chip) {
...
static void rb400_hwcontrol(struct nand_chip *chip, int cmd,
			    unsigned int ctrl) {
...
static uint8_t rb400_read_byte(struct nand_chip *chip)
{
	uint8_t byte = 0;
...
static void rb400_write_buf(struct nand_chip *chip, const uint8_t *buf, int len)
{
	rb400_nandio_write(buf, len);
}

static void rb400_read_buf(struct nand_chip *chip, uint8_t *buf, int len)
{
...
static int rb400_spi_nand_probe(struct spi_device *_spi)
{
	...
	rnand.legacy.cmd_ctrl = rb400_hwcontrol;
	rnand.legacy.dev_ready = rb400_dev_ready;
	rnand.legacy.read_byte = rb400_read_byte;
	rnand.legacy.write_buf = rb400_write_buf;
	rnand.legacy.read_buf = rb400_read_buf;
