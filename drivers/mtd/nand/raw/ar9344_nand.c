...
static uint8_t ar9344_read_byte(struct nand_chip *chip)
{
...
static void ar9344_read_buf(struct nand_chip *chip, uint8_t *buf, int len)
{
	for (; len > 0; --len, ++buf) {
		*buf = ar9344_read_byte(chip);
	}
}

static void ar9344_write_buf(struct nand_chip *chip, const uint8_t *buf, int len)
{
	for (; len > 0; --len, ++buf) {
...
static void ar9344_select_chip(struct nand_chip *chip, int chipnr) {
	CFG(chipnr >= 0 ? GPIO_S0_REG : GPIO_S1_REG) = GPO_NAND_nCE;
	ndelay(500);
}

static void ar9344_ctrl(struct nand_chip *chip, int cmd, unsigned int ctrl)
{
...
static void ar9344_nand_reset(void) {
...
static int ar9344_write_oob(struct nand_chip *chip, int page)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	if (small_block) {
		send_cmd(0x5000);
		dma_buf_ofs = 0;
		ar9344_write_buf(chip, chip->oob_poi, mtd->oobsize);
...
static void ar9344_command(struct nand_chip *chip, unsigned int command,
			   int column, int page)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	...
	if (command == NAND_CMD_PAGEPROG) {
		if (small_block) {
			send_cmd(0);
			write_buf_to_nand(cur_page >> 8, (cur_page & 0xff) << 8,
					  dma_buf_ofs);
		} else {
			write_buf_to_nand(cur_page, 0, dma_buf_ofs);
		}
		return;
	}
	if (!ctrl_init_done) {
...
static int ar9344_wait_rdy(struct nand_chip *chip)
{
...
static int ar9344_nand_probe(struct platform_device *pdev)
{
	...
	rnand.legacy.read_byte = ar9344_read_byte;
	rnand.legacy.read_buf = ar9344_read_buf;
	rnand.legacy.write_buf = ar9344_write_buf;

	rnand.legacy.select_chip = ar9344_select_chip;
	rnand.legacy.cmd_ctrl = ar9344_ctrl;
	rnand.legacy.cmdfunc = ar9344_command;
	rnand.legacy.waitfunc = ar9344_wait_rdy;
...
static int __init ar9344_nand_init(void)
{
	...
	nand_base = ioremap(nand_addr, PAGE_SIZE);    
	cfg_base = ioremap(GPIO_BASE, 0x30000);
