...
static int f35xqa00xg_ecc_get_status(struct spinand_device *spinand,
				       u8 status)
{
	if((status & STATUS_ECC_MASK) == STATUS_ECC_NO_BITFLIPS) {
		/* STATUS_ECC_NO_BITFLIPS */
		return 0;
	}
	else if((status & STATUS_ECC_MASK) == STATUS_ECC_HAS_BITFLIPS) {
...
static const struct spinand_info foresee_spinand_table[] = {
	SPINAND_INFO("F35SQA001G", 0x71,
		     ...
		     SPINAND_HAS_QE_BIT,
		     SPINAND_ECCINFO(&f35xqa00xg_ooblayout,
				     f35xqa00xg_ecc_get_status)),
