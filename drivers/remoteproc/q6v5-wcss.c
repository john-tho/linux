...
static unsigned long dump_timeout_data;
...
#if defined(CONFIG_IPQ_SUBSYSTEM_DUMP)
static void open_timeout_func(struct timer_list *t)
{
...
static void dump_timeout_func(struct timer_list *t)
{
	struct dump_file_private *dfp = (struct dump_file_private *)dump_timeout_data;

	pr_err("Time Out: Q6 crash dump collection failed\n");

	dump_timeout_data = -ETIMEDOUT;
		...
		loff_t *ppos)
{
	...
	if (dump_timeout_data == -ETIMEDOUT)
		return 0;
...
static int crashdump_init(int check, const struct subsys_desc *desc)
{
	 ...
	 */
	timer_setup(&open_timeout, open_timeout_func, 0);
