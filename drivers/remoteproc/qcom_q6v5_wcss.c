...
#if defined(CONFIG_IPQ_SS_DUMP)
unsigned long open_timeout_func_data = 0;
static void open_timeout_func(struct timer_list *tl)
{
	struct q6v5_wcss *wcss = (struct q6v5_wcss *)open_timeout_func_data;
...
unsigned long dump_timeout_func_data = 0;
static void dump_timeout_func(struct timer_list *tl)
{
	struct q6v5_wcss *wcss = (struct q6v5_wcss *)dump_timeout_func_data;

	pr_err("Time Out: Q6 crash dump collection failed\n");

	dump_timeout_func_data = -ETIMEDOUT;
...
static int q6_dump_open(struct inode *inode, struct file *file)
{
	...
	dump_timeout_func_data = (unsigned long)wcss;
	timer_setup(&wcss->dfp->dump_timeout, dump_timeout_func, 0);
		...
		loff_t *ppos)
{
	...
	if (dump_timeout_func_data == -ETIMEDOUT) {
...
static void crashdump_init(struct rproc *rproc, struct rproc_dump_segment *segment, void *dest)
{
	 ...
	 */
	open_timeout_func_data = (unsigned long)wcss;
	timer_setup(&wcss->dfp->open_timeout, open_timeout_func, 0);
	...
	if (dump_timeout_func_data == -ETIMEDOUT) {
		ret = dump_timeout_func_data;
		dump_timeout_func_data = 0;
	}
...
extern int rproc_start_subdevices(struct rproc *rproc);
...
extern void rproc_stop_subdevices(struct rproc *rproc, bool crashed);
...
irqreturn_t q6v5_wcss_wdog_interrupt(int irq, void *data)
{
...
EXPORT_SYMBOL(q6v5_wcss_wdog_interrupt);

irqreturn_t q6v5_wcss_fatal_interrupt(int irq, void *data)
{
	struct qcom_q6v5 *q6v5 = subsys_to_pdata(data);
	size_t len;
	char *msg;
...
EXPORT_SYMBOL(q6v5_wcss_fatal_interrupt);

void q6v5_wcss_panic_handler(const struct subsys_desc *subsys)
{
	struct qcom_q6v5 *q6v5 = subsys_to_pdata(subsys);
...
static int q6v5_wcss_probe(struct platform_device *pdev)
{
	...
	q6v5->subsys_desc.ramdump = crashdump_init_new;
	if (of_property_read_bool(pdev->dev.of_node, "qcom,q6v6"))
		q6v5->subsys_desc.crash_shutdown = q6v5_wcss_panic_handler;
	q6v5->subsys_desc.err_fatal_handler = q6v5_wcss_fatal_interrupt;
	q6v5->subsys_desc.stop_ack_handler = q6v5_ready_interrupt;
	q6v5->subsys_desc.wdog_bite_handler = q6v5_wcss_wdog_interrupt;
