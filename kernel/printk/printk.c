...
void get_some_log(char *buf, int amount) {
	...
	size_t l = 0;
	bool ret = false;
	int cur_seq = clear_seq;
	unsigned total_size = 0;
	int drop_size = 0;

	memset(buf, 0, amount);
	prb_rec_init_rd(&r, &info, buf, amount, NULL, 0);

	logbuf_lock_irq();

	while (prb_read_valid(prb, cur_seq, &r)) {
		cur_seq = r.info->seq + 1;
		total_size += record_print_text(&r, false, false);
		prb_rec_init_rd(&r, &info, buf, amount, NULL, 0);
	}

	cur_seq = clear_seq;
	drop_size = total_size - amount;

	while (prb_read_valid(prb, cur_seq, &r)) {
		cur_seq = r.info->seq + 1;

		l = record_print_text(&r, false, false);
		if (drop_size > 0) {
			drop_size -= l;
		}
		else {
