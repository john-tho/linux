...
static int mfs_readdir(struct file *file, struct dir_context *ctx)
{
	...
	req.id = MFS_ID(file_inode(file)->i_sb);
	req.cmd = CMD_READDIR;
	req.ino = file_inode(file)->i_ino;
	req.offset = ctx->pos;
	req.size = PAGE_SIZE;
			 ...
			 struct page *page, void *fsdata)
{
	...
	post_request(tx, &req, sizeof(req));
	post_request(tx + 1, buf + (pos & (PAGE_SIZE - 1)), blen);
	wait_for_reply(rx);
				...
				struct delayed_call *done)
{
	...
	*((char *) rep + len) = 0;
	set_delayed_call(done, kfree_link, rep);
	return rep->target;
}
