...
u32 yaffs2_find_refresh_block(struct yaffs_dev *dev)
{
	 ...
	 * Fix broken values.
	 */
	if (dev->next_refresh_jiffies == 0)
		dev->next_refresh_jiffies = jiffies;

	if (time_before(jiffies, dev->next_refresh_jiffies))
		return oldest;

	/*
	 * We'll do a refresh this time around....
	 * Find the oldest block and update the refresh jiffies.
	 */
	dev->refresh_count++;
	bi = dev->block_info;
	...
	dev->next_refresh_jiffies = jiffies +
		dev->param.refresh_period * 3600 * HZ / num_blocks;

	yaffs_trace(YAFFS_TRACE_GC,
		    "%s: block %u refresh, %u left, next after %lu mins\n",
		    dev->param.name, oldest, num_blocks,
		    (dev->next_refresh_jiffies - jiffies) / (60 * HZ));
...
int yaffs2_scan_backwards(struct yaffs_dev *dev)
{
	...
	/* clean up summary table between read & write */
	yaffs_summary_clear(dev);

#ifdef CONFIG_YAFFS_ENABLE_COMPATIBILITY_MODE
	/* skipping of current allocation block creates havoc in old yaffs
           versions - they continue allocation from oldest such partial block
	   which messes up sequence id numbers */
	if (dev->param.inband_tags)
#endif
