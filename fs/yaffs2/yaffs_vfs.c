...
unsigned int yaffs_auto_checkpoint = 0;
unsigned int yaffs_gc_control = 1;
unsigned int yaffs_bg_enable = 1;
unsigned int yaffs_auto_select = 1;
unsigned int yaffs_inband_tags = 0;
/* Module Parameters */
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 5, 0))
...
module_param_named(inband_tags, yaffs_inband_tags, uint, 0644);
#else
...
static int yaffs_iterate(struct file *f, struct dir_context *dc)
{
	...
	if (!inode_eq_iversion(inode, f->f_version)) {
		dc->pos = 2;
				      ...
				      struct yaffs_obj *obj)
{
	if (inode && obj) {
		...
		inode->i_atime.tv_sec = (ktime_t) (obj->yst_atime);
		inode->i_atime.tv_nsec = 0;
		inode->i_mtime.tv_sec = (ktime_t) obj->yst_mtime;
		inode->i_mtime.tv_nsec = 0;
		inode->i_ctime.tv_sec = (ktime_t) obj->yst_ctime;
...
static int yaffs_remount_fs(struct super_block *sb, int *flags, char *data)
{
	struct yaffs_dev *dev = yaffs_super_to_dev(sb);
	struct mtd_info *mtd = yaffs_dev_to_mtd(dev);
	int read_only = ((*flags & SB_RDONLY) != 0);
	...
	else if (dev->read_only && !read_only) {
		yaffs_bg_start(dev);
	}
						     ...
						     void *data, int silent)
{
	...
	options.inband_tags = yaffs_inband_tags;
	...
	if ((mtd->oobavail < 22 &&  // 16b tags2 + 4b MLC ecc + 2b MLC marker
	     mtd->oobsize > 16) ||
	    options.inband_tags)
		inband_tags = 1;
...
#else
static ssize_t yaffs_proc_write(struct file *file, const char __user *ubuf,
...
#endif
{
	...
	if (copy_from_user(buf, ubuf, count) > 0) return -EFAULT;
...
static void yaffs_dump_dev_part0(struct seq_file *m, struct yaffs_dev *dev)
{
	...
	seq_printf(m, "n_bad_blocks......... %u\n",
				bs[YAFFS_BLOCK_STATE_DEAD]);
...
static char *yaffs_dump_dev_part0(char *buf, struct yaffs_dev *dev)
{
	...
	buf += sprintf(buf, "n_reserved_blocks.... %d\n",
				param->n_reserved_blocks);
	buf += sprintf(buf, "n_bad_blocks......... %u\n",
				bs[YAFFS_BLOCK_STATE_DEAD]);
