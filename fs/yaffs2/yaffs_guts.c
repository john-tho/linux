...
void yaffs_block_became_dirty(struct yaffs_dev *dev, int block_no)
{
	...
	/*
	 * on slow flash we do the erase in gc only
	 * otherwise rm of big file may take more than 120s
	 * which leads to kernel panic with blocked task message
	 */
	if (dev->param.erase_is_slow && block_no != (int)dev->gc_block) {
		bi->gc_prioritise = 1;
		dev->has_pending_prioritised_gc = 1;
		return;
	}
...
static int yaffs_check_gc(struct yaffs_dev *dev, int background)
{
	...
	do {
		...
		if (dev->gc_block < 1 &&
		    (aggressive || erased_chunks < dev->n_free_chunks / 2 ||
		     (dev->has_pending_prioritised_gc && background))) {
			dev->gc_block =
				   ...
				   enum yaffs_obj_type type)
{
	...
	case YAFFS_OBJECT_TYPE_DIRECTORY:
		/*
		 * Never change directory into file as it may cause panic by
		 * Trying to add an object to a non-directory
		 */
		return obj;
