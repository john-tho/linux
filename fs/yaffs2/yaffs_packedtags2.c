...
int yaffs_pack_tags2(struct yaffs_dev *dev,
		      struct yaffs_packed_tags2 *pt,
		      const struct yaffs_ext_tags *t,
		      int tags_ecc, int max_packed_size)
{
	yaffs_pack_tags2_tags_only(dev, &pt->t, t);
	if (!tags_ecc)
		return sizeof(pt->t);

	if (max_packed_size < sizeof(struct yaffs_packed_tags2)) {
		*(5 + (unsigned char *)&pt->ecc) = 0x55;
		yaffs_ecc_calc_mlc_other((unsigned char *)&pt->t);
		return max_packed_size;
	}
	else {
...
void yaffs_unpack_tags2(struct yaffs_dev *dev,
			struct yaffs_ext_tags *t,
			struct yaffs_packed_tags2 *pt,
			int tags_ecc, int packed_tags_size)
{
	...
	if (pt->t.seq_number != 0xffffffff && tags_ecc) {
		...
		if (packed_tags_size < sizeof(struct yaffs_packed_tags2)) {
			result = yaffs_ecc_correct_mlc_other(
				(unsigned char *)&pt->t);
		}
		else
		{
